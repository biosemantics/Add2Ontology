<template>
    <div slot="section" class="">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3>Wizard</h3>
            </div>
            <div class="panel-body table-responsive" style="font-size: 15px;">
                <div class="tab-pane">
                    <form class="form-horizontal" autocomplete="off">
                        <div v-if="status != 5" class="form-group">
                            <label class="col-md-8 col-md-offset-2">
                                Answer the following questions about [term to be added]. Your answer helps the computer understand the meaning of the term.
                            </label>
                        </div>
                        <div v-if="status == 0">
                            <div class="form-group">
                                <div class="col-md-8 col-md-offset-2">
                                    <label>Please provide  a human readable definition for [term to be added].</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input v-model="term.definition" style="width: 100%;"/>
                                </div>
                            </div>

                            <div class="form-group">
                                <div class="col-md-8 col-md-offset-2">
                                    <a class="btn btn-primary" v-on:click="submit(0)">Save</a>
                                </div>
                            </div>
                        </div>


                        <div v-if="status == 1">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-8">
                                    <label>Does [term to be added] represent</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question1-y" v-model="term.represent" value="anatomical" name="structure" />
                                    <label for="question1-y">an anatomical structure? </label>
                                    <div style="padding-left: 10px;">
                                        If the term represents a physical object such as leaf, apex, pore, or imaginary anatomical structures such as axis, then choose “anatomical structure”.  Example anatomical structure terms (link to Carex Ontology anatomical entity branch on WebProtege)
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question1-n" v-model="term.represent" value="character" name="structure" />
                                    <label for="question1-n">a character?</label>
                                    <div style="padding-left: 10px;">
                                        If the term represents a property such as color, shape, size, orientation, position etc. of a physical object, then choose “character”. Example character terms (link to Carex Ontology quality branch on WebProtege)
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-8 col-md-offset-2">
                                    <a class="btn btn-primary" v-on:click="submit(1)">Save</a>
                                </div>

                            </div>
                        </div>

                        <div v-if="status == 2">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-8">
                                    <label>Is [term to be added] a synonym to any of the existing anatomical structure terms? </label>
                                </div>
                                <div class="col-md-offset-2 col-md-4">
                                    <!--<v-jstree :data="treeData" show-checkbox multiple allow-batch whole-row @item-click="synonymItemClick"></v-jstree>-->
                                    <tree
                                            :data="treeData"
                                            :options="synonymsOption"
                                            ref="tree"
                                            @node:checked="onSynonymSelected"
                                            @node:unchecked="onSynonymUnselected"
                                    />
                                </div>
                                <div class="col-md-4">
                                    <pre>{{ treeDetails.q2 }}</pre>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question2-y" v-model="term.synonym" value="yes" name="structure" />
                                    <label for="question2-y">Yes </label>
                                    <div style="padding-left: 10px;">
                                        In the tree above, select the nodes that [term to be added] is a synonym of.
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question2-n" v-model="term.synonym" value="no" name="structure" />
                                    <label for="question2-n">No</label>
                                    <div style="padding-left: 10px;">
                                        In the tree above, select the nodes under which [term to be added] should be added.
                                    </div>
                                </div>
                                <div v-if="status == 2" class="col-md-8 col-md-offset-2">
                                    <a class="btn btn-primary" v-on:click="submit(2)">Save</a>
                                </div>
                            </div>
                        </div>
                        <div v-if="status == -1">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-8">
                                    <label>Is [term to be added] a synonym to any of the existing quality terms? </label>
                                </div>
                                <div class="col-md-offset-2 col-md-4">
                                    <!--<v-jstree :data="treeData" show-checkbox multiple allow-batch whole-row @item-click="synonymItemClick"></v-jstree>-->
                                    <tree
                                            :data="treeData"
                                            :options="synonymsOption"
                                            ref="tree"
                                            @node:checked="onCharacterSynonymSelected"
                                            @node:unchecked="onSynonymUnselected"
                                    />
                                </div>
                                <div class="col-md-4">
                                    <pre>{{ treeDetails.qx }}</pre>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="questionx-y" v-model="term.synonym" value="yes" name="structure" />
                                    <label for="questionx-y">Yes </label>
                                    <div style="padding-left: 10px;">
                                        In the tree above, select the nodes that [term to be added] is a synonym of.
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="questionx-n" v-model="term.synonym" value="no" name="structure" />
                                    <label for="questionx-n">No</label>
                                    <div style="padding-left: 10px;">
                                        In the tree above, select the nodes under which [term to be added] should be added.
                                    </div>
                                </div>
                                <div v-if="status == -1" class="col-md-8 col-md-offset-2">
                                    <a class="btn btn-primary" v-on:click="submit(-1)">Save</a>
                                </div>
                            </div>
                        </div>
                        <div v-if="status == 3">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-8">
                                    <label>Are all instances of [term to be added] part of some larger structures shown in the tree below? </label>
                                </div>
                                <div class="col-md-offset-2 col-md-4">
                                    <tree
                                            :data="treeData"
                                            :options="synonymsOption"
                                            ref="tree"
                                            @node:checked="onPartOfSelected"
                                            @node:unchecked="onSynonymUnselected"
                                    />
                                </div>
                                <div class="col-md-4">
                                    <pre>{{ treeDetails.q3 }}</pre>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question3-y" v-model="term.instance" value="yes" name="structure" />
                                    <label for="question3-y">Yes </label>
                                    <div style="padding-left: 10px;">
                                        Select the larger structures to which all instances of [term to be added] belong.
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question3-n" v-model="term.instance" value="no" name="structure" />
                                    <label for="question3-n"> No, I am not aware of such larger structures.</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question3-n-user" v-model="term.instance" value="no-user" name="structure" />
                                    <label for="question3-n-user">No, such larger structures are not shown in the tree. Here are these structures and their definitions:</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2" v-if="term.instance == 'no-user'">
                                    <div style="padding-left: 10px;">
                                        <div class="row">
                                            <div class="col-md-4">
                                                Term
                                            </div>
                                            <div class="col-md-4">
                                                Human readable definition
                                            </div>
                                        </div>
                                        <div class="row" v-for="i in userInstances.length" style="padding-top: 5px;">
                                            <div class="col-md-4">
                                                <input v-model="userInstances[i-1].term"/>
                                            </div>
                                            <div class="col-md-4">
                                                <input v-model="userInstances[i-1].definition"/>
                                            </div>
                                        </div>
                                    </div>
                                    <div style="padding-left: 10px; margin-top: 20px;">
                                        <a class="btn btn-primary "  v-on:click="addUserDefinition()"><span class="glyphicon glyphicon-plus"></span></a>
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2" style="margin-top: 20px;">
                                    <a class="btn btn-primary" v-on:click="submit(3)">Save</a>
                                </div>
                            </div>
                        </div>
                        <div v-if="status == 4">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-8">
                                    <label>Are all instances of [term to be added] have certain parts that are in the tree below? </label>
                                </div>
                                <div class="col-md-offset-2 col-md-4">
                                    <tree
                                            :data="treeData"
                                            :options="synonymsOption"
                                            ref="tree"
                                            @node:checked="onHasPartSelected"
                                            @node:unchecked="onSynonymUnselected"
                                    />
                                </div>
                                <div class="col-md-4">
                                    <pre>{{ treeDetails.q4 }}</pre>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question4-y" v-model="term.hasPart" value="yes" name="structure" />
                                    <label for="question4-y">Yes </label>
                                    <div style="padding-left: 10px;">
                                        Select the parts.
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question4-n" v-model="term.hasPart" value="no" name="structure" />
                                    <label for="question4-n"> No, I am not aware of such smaller structures.</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2">
                                    <input type="radio" id="question4-n-user" v-model="term.hasPart" value="no-user" name="structure" />
                                    <label for="question4-n-user">No, such smaller structures are not shown in the tree. Here are these structures and their definitions:</label>
                                </div>
                                <div class="col-md-8 col-md-offset-2" v-if="term.hasPart == 'no-user'">
                                    <div style="padding-left: 10px;">
                                        <div class="row">
                                            <div class="col-md-4">
                                                Term
                                            </div>
                                            <div class="col-md-4">
                                                Human readable definition
                                            </div>
                                        </div>
                                        <div class="row" v-for="i in userHasParts.length" style="padding-top: 5px;">
                                            <div class="col-md-4">
                                                <input v-model="userHasParts[i-1].term"/>
                                            </div>
                                            <div class="col-md-4">
                                                <input v-model="userHasParts[i-1].definition"/>
                                            </div>
                                        </div>
                                    </div>
                                    <div style="padding-left: 10px; margin-top: 20px;">
                                        <a class="btn btn-primary "  v-on:click="addUserHasPart()"><span class="glyphicon glyphicon-plus"></span></a>
                                    </div>
                                </div>
                                <div class="col-md-8 col-md-offset-2" style="margin-top: 20px;">
                                    <a class="btn btn-primary" v-on:click="submit(4)">Save</a>
                                </div>
                            </div>
                        </div>
                        <div v-if="status == 5" class="col-md-offset-2 col-md-8">
                            <div v-for="each in summary">
                                {{ each }}
                            </div>
                            <a class="btn btn-primary" v-on:click="Submit('final')" style="margin-top: 10px;">Submit</a>
                        </div>
                        <div v-if="modalShowFlag" @close="modalShowFlag = false">
                            <transition name="modal">
                                <div class="modal-mask">
                                    <div class="modal-wrapper">
                                        <div class="modal-container">

                                            <div class="modal-header">
                                                <h3>Confirm Dialog</h3>
                                            </div>

                                            <div class="modal-body">
                                                <div class="row">
                                                    <div v-if="status == 2 || status == -1" class="col-md-12">
                                                        <div v-if="term.synonym == 'yes'">
                                                            You added [term to be added] as synonyms to
                                                            <div v-for="each in synonyms">
                                                                {{ each.text }}
                                                            </div>
                                                        </div>
                                                        <div v-if="term.synonym == 'no'">
                                                            You added [term to be added] as subclass of
                                                            <div v-for="each in synonyms">
                                                                {{ each.text }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div v-if="status == 3" class="col-md-12">
                                                        <div v-if="term.instance == 'yes'">
                                                            Do you want to add part_of relation between {{temp.text}} and LSTs:
                                                            <div v-for="each in instances">
                                                                {{ each.text }}
                                                            </div>
                                                        </div>
                                                        <div v-if="term.instance == 'no-user'">
                                                            Do you want to add part_of relation between {{temp.text}} and LSTs:
                                                            <div v-for="each in instances">
                                                                {{ each.term }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div v-if="status == 4" class="col-md-12">
                                                        <div v-if="term.instance == 'yes'">
                                                            Do you want to add has_part relation between {{temp.text}} and LSTs:
                                                            <div v-for="each in hasParts">
                                                                {{ each.text }}
                                                            </div>
                                                        </div>
                                                        <div v-if="term.instance == 'no-user'">
                                                            Do you want to add has_part relation between {{temp.text}} and LSTs:
                                                            <div v-for="each in hasParts">
                                                                {{ each.term }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-md-12 text-right" style="margin-top: 15px;">
                                                        <a v-on:click="confirmSave(status)" class="btn btn-primary">Confirm</a>
                                                        <a v-on:click="cancelSave()" class="btn btn-danger">Undo</a>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="modal-footer">

                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </transition>
                        </div>

                    </form>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    import VJstree from 'vue-jstree';
    import Vue from 'vue';
    import LiquorTree from 'liquor-tree'

    Vue.use(LiquorTree);
    Vue.use(VJstree);


    export default {
        data: function() {
            return {
                username: sessionStorage.getItem('username'),
                status: 0,
                term: {
                    definition: null,
                    represent: null,
                    synonym: null,
                    instance: null,
                    hasPart: null,

                },
                treeDetails: {
                    q2: null,
                    qx: null,
                    q3: null,
                    q4: null
                },
                arraySynonyms: [],
                treeData: [

                ],
                synonymsOption:{
                    checkbox: true,
                    autoCheckChildren: false,
                    parentSelect: true,
                    checkOnSelect: true
                },
                synonyms: [],
                instances: [],
                hasParts: [],
                modalShowFlag: false,
                temp : {
                    text: 'test11',
                    state: {
                        checked: false
                    },
                    data: {
                        details: [
                            {}
                        ]
                    }
                },
                userInstances: [
                    {
                        term: null,
                        definition: null
                    }
                ],
                userHasParts: [
                    {
                        term: null,
                        definition: null
                    }
                ],
                tempIndex: 0,
                tempJ: 0,
                TTBA: [],
                largeDataEntity: {
                    "data": {
                        "details": [
                            {
                                "IRI": "http://purl.obolibrary.org/obo/UBERON_0001062"
                            }
                        ]
                    },
                    "children": [
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inflorescence"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#raceme"
                                            }
                                        ]
                                    },
                                    "text": "raceme"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20inflorescence"
                                            }
                                        ]
                                    },
                                    "text": "distal inflorescence"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spike%20inflorescence"
                                            }
                                        ]
                                    },
                                    "text": "spike inflorescence"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cyme"
                                            }
                                        ]
                                    },
                                    "text": "cyme"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#corymb"
                                            }
                                        ]
                                    },
                                    "text": "corymb"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#primary%20inflorescence"
                                            }
                                        ]
                                    },
                                    "text": "primary inflorescence"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#panicle"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#secondary%20inflorescences%20panicle"
                                                    }
                                                ]
                                            },
                                            "text": "secondary inflorescences panicle"
                                        }
                                    ],
                                    "text": "panicle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#secondary%20inflorescence"
                                            }
                                        ]
                                    },
                                    "text": "secondary inflorescence"
                                }
                            ],
                            "text": "inflorescence"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#vein",
                                        "exact synonyms": "veins"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#finer%20vein"
                                            }
                                        ]
                                    },
                                    "text": "finer vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lateral-vein"
                                            }
                                        ]
                                    },
                                    "text": "lateral-vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#midvein"
                                            }
                                        ]
                                    },
                                    "text": "midvein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#main%20vein"
                                            }
                                        ]
                                    },
                                    "text": "main vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lateral%20adaxial%20vein"
                                            }
                                        ]
                                    },
                                    "text": "lateral adaxial vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cross-vein"
                                            }
                                        ]
                                    },
                                    "text": "cross-vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#marginal%20vein"
                                            }
                                        ]
                                    },
                                    "text": "marginal vein"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20vein"
                                            }
                                        ]
                                    },
                                    "text": "adaxial vein"
                                }
                            ],
                            "text": "vein"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#collar"
                                    }
                                ]
                            },
                            "text": "collar"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tuft"
                                    }
                                ]
                            },
                            "text": "tuft"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20apex"
                                    }
                                ]
                            },
                            "text": "sheath apex"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20leaf-sheath"
                                    }
                                ]
                            },
                            "text": "basal leaf-sheath"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath-blade%20junction"
                                    }
                                ]
                            },
                            "text": "sheath-blade junction"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#filament"
                                    }
                                ]
                            },
                            "text": "filament"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#septum"
                                    }
                                ]
                            },
                            "text": "septum"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#endosperm"
                                    }
                                ]
                            },
                            "text": "endosperm"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tussock"
                                    }
                                ]
                            },
                            "text": "tussock"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#venation"
                                    }
                                ]
                            },
                            "text": "venation"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#teeth"
                                    }
                                ]
                            },
                            "text": "teeth"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perianth"
                                    }
                                ]
                            },
                            "text": "perianth"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaves%20blade"
                                    }
                                ]
                            },
                            "text": "leaves blade"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shoulder"
                                    }
                                ]
                            },
                            "text": "shoulder"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stigma"
                                    }
                                ]
                            },
                            "text": "stigma"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#testa"
                                    }
                                ]
                            },
                            "text": "testa"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#seed"
                                    }
                                ]
                            },
                            "text": "seed"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#keels"
                                    }
                                ]
                            },
                            "text": "keels"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20front"
                                    }
                                ]
                            },
                            "text": "sheath front"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak%20apex"
                                            }
                                        ]
                                    },
                                    "text": "beak apex"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak%20orifice"
                                            }
                                        ]
                                    },
                                    "text": "beak orifice"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak%20tooth"
                                            }
                                        ]
                                    },
                                    "text": "beak tooth"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak%20tip"
                                            }
                                        ]
                                    },
                                    "text": "beak tip"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20beak"
                                            }
                                        ]
                                    },
                                    "text": "distal beak"
                                }
                            ],
                            "text": "beak"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cone"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slender%20terminal%20cone"
                                            }
                                        ]
                                    },
                                    "text": "slender terminal cone"
                                }
                            ],
                            "text": "cone"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20scale"
                                            }
                                        ]
                                    },
                                    "text": "proximal scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subtending%20scale"
                                            }
                                        ]
                                    },
                                    "text": "subtending scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stamimate%20scale"
                                            }
                                        ]
                                    },
                                    "text": "stamimate scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enfolding%20scale"
                                            }
                                        ]
                                    },
                                    "text": "enfolding scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20scale"
                                            }
                                        ]
                                    },
                                    "text": "longest scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20scale"
                                            }
                                        ]
                                    },
                                    "text": "basal scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximalmost%20scale"
                                            }
                                        ]
                                    },
                                    "text": "proximalmost scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle%20scale"
                                            }
                                        ]
                                    },
                                    "text": "middle scale"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20scale"
                                            }
                                        ]
                                    },
                                    "text": "distal scale"
                                }
                            ],
                            "text": "scale"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#style%20base"
                                    }
                                ]
                            },
                            "text": "style base"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blade"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bladeless"
                                            }
                                        ]
                                    },
                                    "text": "bladeless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20leaf-blade"
                                            }
                                        ]
                                    },
                                    "text": "widest leaf-blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf-blade",
                                                "exact synonyms": "leaves blade"
                                            }
                                        ]
                                    },
                                    "text": "leaf-blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger%20blade"
                                            }
                                        ]
                                    },
                                    "text": "larger blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20blade"
                                            }
                                        ]
                                    },
                                    "text": "proximal blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20leaf-blade"
                                            }
                                        ]
                                    },
                                    "text": "distal leaf-blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#largest%20blade"
                                            }
                                        ]
                                    },
                                    "text": "largest blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20leaf-blade"
                                            }
                                        ]
                                    },
                                    "text": "proximal leaf-blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline%20blade"
                                            }
                                        ]
                                    },
                                    "text": "cauline blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20blade"
                                            }
                                        ]
                                    },
                                    "text": "widest blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20bract%20blade"
                                            }
                                        ]
                                    },
                                    "text": "widest bract blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20bract%20blade"
                                            }
                                        ]
                                    },
                                    "text": "longest bract blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bladed"
                                            }
                                        ]
                                    },
                                    "text": "bladed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bract%20blade"
                                            }
                                        ]
                                    },
                                    "text": "bract blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20blade"
                                            }
                                        ]
                                    },
                                    "text": "sheath blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blade%20surface"
                                            }
                                        ]
                                    },
                                    "text": "blade surface"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20bract%20blade"
                                            }
                                        ]
                                    },
                                    "text": "distal bract blade"
                                }
                            ],
                            "text": "blade"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale%20body"
                                    }
                                ]
                            },
                            "text": "scale body"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale%20apex"
                                    }
                                ]
                            },
                            "text": "scale apex"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rootlets"
                                    }
                                ]
                            },
                            "text": "rootlets"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#branch"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20branch"
                                            }
                                        ]
                                    },
                                    "text": "proximal branch"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20branch"
                                            }
                                        ]
                                    },
                                    "text": "basal branch"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20lateral%20branch"
                                            }
                                        ]
                                    },
                                    "text": "proximal lateral branch"
                                }
                            ],
                            "text": "branch"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#midrib"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20midrib"
                                            }
                                        ]
                                    },
                                    "text": "abaxial midrib"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#central%20midrib"
                                            }
                                        ]
                                    },
                                    "text": "central midrib"
                                }
                            ],
                            "text": "midrib"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20summits%20collar"
                                    }
                                ]
                            },
                            "text": "sheath summits collar"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#disk"
                                    }
                                ]
                            },
                            "text": "disk"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#keel",
                                        "exact synonyms": "keels"
                                    }
                                ]
                            },
                            "text": "keel"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#node",
                                        "exact synonyms": "nodes"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhizome%20internode"
                                            }
                                        ]
                                    },
                                    "text": "rhizome internode"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle%20internode"
                                            }
                                        ]
                                    },
                                    "text": "middle internode"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20node"
                                            }
                                        ]
                                    },
                                    "text": "basal node"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline%20node"
                                            }
                                        ]
                                    },
                                    "text": "cauline node"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#internode"
                                            }
                                        ]
                                    },
                                    "text": "internode"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20internode"
                                            }
                                        ]
                                    },
                                    "text": "longest internode"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20internode"
                                            }
                                        ]
                                    },
                                    "text": "proximal internode"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20proximal%20internode"
                                            }
                                        ]
                                    },
                                    "text": "longest proximal internode"
                                }
                            ],
                            "text": "node"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20front%20apex"
                                    }
                                ]
                            },
                            "text": "sheath front apex"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#suture"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20suture"
                                            }
                                        ]
                                    },
                                    "text": "abaxial suture"
                                }
                            ],
                            "text": "suture"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf%20ligule"
                                    }
                                ]
                            },
                            "text": "leaf ligule"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wing",
                                        "exact synonyms": "wings"
                                    }
                                ]
                            },
                            "text": "wing"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20beak%20tip"
                                    }
                                ]
                            },
                            "text": "perigynium beak tip"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#style"
                                    }
                                ]
                            },
                            "text": "style"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale%20scale"
                                    }
                                ]
                            },
                            "text": "scale scale"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shoot"
                                    }
                                ]
                            },
                            "text": "shoot"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ligule",
                                        "exact synonyms": "leaf ligule"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20ligule"
                                            }
                                        ]
                                    },
                                    "text": "longest ligule"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20ligule"
                                            }
                                        ]
                                    },
                                    "text": "distal ligule"
                                }
                            ],
                            "text": "ligule"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pistil"
                                    }
                                ]
                            },
                            "text": "pistil"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#margin"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20margin"
                                            }
                                        ]
                                    },
                                    "text": "proximal margin"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20margin"
                                            }
                                        ]
                                    },
                                    "text": "abaxial margin"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apical%20margin"
                                            }
                                        ]
                                    },
                                    "text": "apical margin"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20margin"
                                            }
                                        ]
                                    },
                                    "text": "distal margin"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enfolding%20margin"
                                            }
                                        ]
                                    },
                                    "text": "enfolding margin"
                                }
                            ],
                            "text": "margin"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apices"
                                    }
                                ]
                            },
                            "text": "apices"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stalk"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stipe"
                                            }
                                        ]
                                    },
                                    "text": "stipe"
                                }
                            ],
                            "text": "stalk"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#peduncle"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20peduncle"
                                            }
                                        ]
                                    },
                                    "text": "distal peduncle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longer%20peduncle"
                                            }
                                        ]
                                    },
                                    "text": "longer peduncle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spike%20peduncle"
                                            }
                                        ]
                                    },
                                    "text": "spike peduncle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20peduncle"
                                            }
                                        ]
                                    },
                                    "text": "proximal peduncle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slender%20peduncle"
                                            }
                                        ]
                                    },
                                    "text": "slender peduncle"
                                }
                            ],
                            "text": "peduncle"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flower"
                                    }
                                ]
                            },
                            "text": "flower"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tomentum"
                                    }
                                ]
                            },
                            "text": "tomentum"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "cauline leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#culm%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "culm leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#previous%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "previous leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "distal leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "basal leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20cauline%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "proximal cauline leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "larger leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "proximal leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cataphyll"
                                            }
                                        ]
                                    },
                                    "text": "cataphyll"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20cauline%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "distal cauline leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "widest leaf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost%20leaf"
                                            }
                                        ]
                                    },
                                    "text": "distalmost leaf"
                                }
                            ],
                            "text": "leaf"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#costa"
                                    }
                                ]
                            },
                            "text": "costa"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#root"
                                    }
                                ]
                            },
                            "text": "root"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#neck"
                                    }
                                ]
                            },
                            "text": "neck"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bristle"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bristle%20tip"
                                            }
                                        ]
                                    },
                                    "text": "bristle tip"
                                }
                            ],
                            "text": "bristle"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf%20sheath"
                                    }
                                ]
                            },
                            "text": "leaf sheath"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20summit"
                                    }
                                ]
                            },
                            "text": "sheath summit"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#non-specific%20material%20anatomical%20entity"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fiber"
                                            }
                                        ]
                                    },
                                    "text": "fiber"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lines"
                                            }
                                        ]
                                    },
                                    "text": "lines"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#end"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20end"
                                                    }
                                                ]
                                            },
                                            "text": "proximal end"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20end"
                                                    }
                                                ]
                                            },
                                            "text": "distal end"
                                        }
                                    ],
                                    "text": "end"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tip"
                                            }
                                        ]
                                    },
                                    "text": "tip"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#area"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#intervenal%20area"
                                                    }
                                                ]
                                            },
                                            "text": "intervenal area"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20area"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial area"
                                        }
                                    ],
                                    "text": "area"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#center"
                                            }
                                        ]
                                    },
                                    "text": "center"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#surface"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20surface"
                                                    }
                                                ]
                                            },
                                            "text": "abaxial surface"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20surface"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial surface"
                                        }
                                    ],
                                    "text": "surface"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#top"
                                            }
                                        ]
                                    },
                                    "text": "top"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#system"
                                            }
                                        ]
                                    },
                                    "text": "system"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#side"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20side"
                                                    }
                                                ]
                                            },
                                            "text": "abaxial side"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20side"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial side"
                                        }
                                    ],
                                    "text": "side"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cavity"
                                            }
                                        ]
                                    },
                                    "text": "cavity"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#attachment"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stalked"
                                                    }
                                                ]
                                            },
                                            "text": "stalked"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sessile"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subsessile"
                                                            }
                                                        ]
                                                    },
                                                    "text": "subsessile"
                                                }
                                            ],
                                            "text": "sessile"
                                        }
                                    ],
                                    "text": "attachment"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#edge"
                                            }
                                        ]
                                    },
                                    "text": "edge"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#segment"
                                            }
                                        ]
                                    },
                                    "text": "segment"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flap"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20flap"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial flap"
                                        }
                                    ],
                                    "text": "flap"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#band"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20band"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial band"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#submarginal%20band"
                                                    }
                                                ]
                                            },
                                            "text": "submarginal band"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subapical%20band"
                                                    }
                                                ]
                                            },
                                            "text": "subapical band"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inner%20band"
                                                    }
                                                ]
                                            },
                                            "text": "inner band"
                                        }
                                    ],
                                    "text": "band"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orifice"
                                            }
                                        ]
                                    },
                                    "text": "orifice"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#direction"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#back"
                                                    }
                                                ]
                                            },
                                            "text": "back"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#horizontally"
                                                    }
                                                ]
                                            },
                                            "text": "horizontally"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#front"
                                                    }
                                                ]
                                            },
                                            "text": "front"
                                        }
                                    ],
                                    "text": "direction"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#patch"
                                            }
                                        ]
                                    },
                                    "text": "patch"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#groove"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#median%20adaxial%20groove"
                                                    }
                                                ]
                                            },
                                            "text": "median adaxial groove"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#median%20groove"
                                                    }
                                                ]
                                            },
                                            "text": "median groove"
                                        }
                                    ],
                                    "text": "groove"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#projection"
                                            }
                                        ]
                                    },
                                    "text": "projection"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#remnant"
                                            }
                                        ]
                                    },
                                    "text": "remnant"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#part"
                                            }
                                        ]
                                    },
                                    "text": "part"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#junction"
                                            }
                                        ]
                                    },
                                    "text": "junction"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apex",
                                                "exact synonyms": "apices"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ultimate%20apex"
                                                    }
                                                ]
                                            },
                                            "text": "ultimate apex"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#front%20apex"
                                                    }
                                                ]
                                            },
                                            "text": "front apex"
                                        }
                                    ],
                                    "text": "apex"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#filling"
                                            }
                                        ]
                                    },
                                    "text": "filling"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#covering"
                                            }
                                        ]
                                    },
                                    "text": "covering"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tissue"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20tissue"
                                                    }
                                                ]
                                            },
                                            "text": "basal tissue"
                                        }
                                    ],
                                    "text": "tissue"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#point"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20point"
                                                    }
                                                ]
                                            },
                                            "text": "widest point"
                                        }
                                    ],
                                    "text": "point"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#outline"
                                            }
                                        ]
                                    },
                                    "text": "outline"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#body"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20body"
                                                    }
                                                ]
                                            },
                                            "text": "distal body"
                                        }
                                    ],
                                    "text": "body"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#head"
                                            }
                                        ]
                                    },
                                    "text": "head"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#face",
                                                "exact synonyms": "faces"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial%20face"
                                                    }
                                                ]
                                            },
                                            "text": "abaxial face"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial%20face"
                                                    }
                                                ]
                                            },
                                            "text": "adaxial face"
                                        }
                                    ],
                                    "text": "face"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#region"
                                            }
                                        ]
                                    },
                                    "text": "region"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#portion"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#median%20portion"
                                                    }
                                                ]
                                            },
                                            "text": "median portion"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle%20and%20proximal%20portion"
                                                    }
                                                ]
                                            },
                                            "text": "middle and proximal portion"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20portion"
                                                    }
                                                ]
                                            },
                                            "text": "distal portion"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20portion"
                                                    }
                                                ]
                                            },
                                            "text": "proximal portion"
                                        }
                                    ],
                                    "text": "portion"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cross-section"
                                            }
                                        ]
                                    },
                                    "text": "cross-section"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#base"
                                            }
                                        ]
                                    },
                                    "text": "base"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#extension"
                                            }
                                        ]
                                    },
                                    "text": "extension"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#depression"
                                            }
                                        ]
                                    },
                                    "text": "depression"
                                }
                            ],
                            "text": "non-specific material anatomical entity"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#foliage%20leaf-blade"
                                    }
                                ]
                            },
                            "text": "foliage leaf-blade"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20mouth"
                                    }
                                ]
                            },
                            "text": "perigynium mouth"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bract"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20bract"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20nonbasal%20bract"
                                                    }
                                                ]
                                            },
                                            "text": "proximal nonbasal bract"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20proximal%20bract"
                                                    }
                                                ]
                                            },
                                            "text": "longest proximal bract"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20cauline%20bract"
                                                    }
                                                ]
                                            },
                                            "text": "proximal cauline bract"
                                        }
                                    ],
                                    "text": "proximal bract"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glume"
                                            }
                                        ]
                                    },
                                    "text": "glume"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost%20lateral%20bract"
                                            }
                                        ]
                                    },
                                    "text": "distalmost lateral bract"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20bract"
                                            }
                                        ]
                                    },
                                    "text": "longest bract"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inflorescence%20bract"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#primary%20inflorescence%20bract"
                                                    }
                                                ]
                                            },
                                            "text": "primary inflorescence bract"
                                        }
                                    ],
                                    "text": "inflorescence bract"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20bract"
                                            }
                                        ]
                                    },
                                    "text": "distal bract"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bractlet"
                                            }
                                        ]
                                    },
                                    "text": "bractlet"
                                }
                            ],
                            "text": "bract"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pericarp"
                                    }
                                ]
                            },
                            "text": "pericarp"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tooth"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#marginal%20tooth"
                                            }
                                        ]
                                    },
                                    "text": "marginal tooth"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apical%20tooth"
                                            }
                                        ]
                                    },
                                    "text": "apical tooth"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beak%20tooth"
                                            }
                                        ]
                                    },
                                    "text": "beak tooth"
                                }
                            ],
                            "text": "tooth"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cortex"
                                    }
                                ]
                            },
                            "text": "cortex"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20base"
                                    }
                                ]
                            },
                            "text": "perigynium base"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spike"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "basal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "distal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost%20spike"
                                            }
                                        ]
                                    },
                                    "text": "distalmost spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "proximal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spike%20portion"
                                            }
                                        ]
                                    },
                                    "text": "spike portion"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "terminal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger%20spike"
                                            }
                                        ]
                                    },
                                    "text": "larger spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longer%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "longer lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20spike"
                                            }
                                        ]
                                    },
                                    "text": "longest spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20proximal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "longest proximal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20and%20distal%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "terminal and distal lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "distal lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "distalmost lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "proximal lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distlmost%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "distlmost lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#portion%20spike"
                                            }
                                        ]
                                    },
                                    "text": "portion spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20cauline%20spike"
                                            }
                                        ]
                                    },
                                    "text": "proximal cauline spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spikelet"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20spikelet"
                                                    }
                                                ]
                                            },
                                            "text": "terminal spikelet"
                                        }
                                    ],
                                    "text": "spikelet"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "longest lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximalmost%20spike"
                                            }
                                        ]
                                    },
                                    "text": "proximalmost spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spike%20set"
                                            }
                                        ]
                                    },
                                    "text": "spike set"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "terminal lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20lateral%20spike"
                                            }
                                        ]
                                    },
                                    "text": "basal lateral spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline%20spike"
                                            }
                                        ]
                                    },
                                    "text": "cauline spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bract%20spike"
                                            }
                                        ]
                                    },
                                    "text": "bract spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20or%20proximal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "terminal or proximal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle%20spike"
                                            }
                                        ]
                                    },
                                    "text": "middle spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20terminal%20spike"
                                            }
                                        ]
                                    },
                                    "text": "longest terminal spike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#individual%20spike"
                                            }
                                        ]
                                    },
                                    "text": "individual spike"
                                }
                            ],
                            "text": "spike"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf-sheath"
                                    }
                                ]
                            },
                            "text": "leaf-sheath"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath",
                                        "exact synonyms": "leaf-sheath"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "distalmost sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20and%20proximal%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "basal and proximal sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal%20sheath",
                                                "exact synonyms": "basal leaf-sheath"
                                            }
                                        ]
                                    },
                                    "text": "basal sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "proximal sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nonbasal%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "nonbasal sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bract%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "bract sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "distal sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inflorescence%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "inflorescence sheath"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20bract%20sheath"
                                            }
                                        ]
                                    },
                                    "text": "proximal bract sheath"
                                }
                            ],
                            "text": "sheath"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fruit"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#achene"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#achene%20body"
                                                    }
                                                ]
                                            },
                                            "text": "achene body"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#achenes%20side"
                                                    }
                                                ]
                                            },
                                            "text": "achenes side"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#achene%20apex"
                                                    }
                                                ]
                                            },
                                            "text": "achene apex"
                                        }
                                    ],
                                    "text": "achene"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nutlet"
                                            }
                                        ]
                                    },
                                    "text": "nutlet"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#utricle"
                                            }
                                        ]
                                    },
                                    "text": "utricle"
                                }
                            ],
                            "text": "fruit"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awn"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apex%20awn"
                                            }
                                        ]
                                    },
                                    "text": "apex awn"
                                }
                            ],
                            "text": "awn"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#anther"
                                    }
                                ]
                            },
                            "text": "anther"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf-base"
                                    }
                                ]
                            },
                            "text": "leaf-base"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#Whole%20Organism"
                                    }
                                ]
                            },
                            "text": "Whole Organism"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#embryo"
                                    }
                                ]
                            },
                            "text": "embryo"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nodes"
                                    }
                                ]
                            },
                            "text": "nodes"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20body"
                                    }
                                ]
                            },
                            "text": "perigynium body"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale%20tip"
                                    }
                                ]
                            },
                            "text": "scale tip"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#summit"
                                    }
                                ]
                            },
                            "text": "summit"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rachis"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rachilla"
                                            }
                                        ]
                                    },
                                    "text": "rachilla"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rachillum"
                                            }
                                        ]
                                    },
                                    "text": "rachillum"
                                }
                            ],
                            "text": "rachis"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prophyll"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "distal perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lateral%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "lateral perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "proximal perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "terminal perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "middle perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#concealing%20perigynium"
                                                    }
                                                ]
                                            },
                                            "text": "concealing perigynium"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20achene"
                                                    }
                                                ]
                                            },
                                            "text": "perigynium achene"
                                        }
                                    ],
                                    "text": "perigynium"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cladoprophyll"
                                            }
                                        ]
                                    },
                                    "text": "cladoprophyll"
                                }
                            ],
                            "text": "prophyll"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stem"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#culm"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tallest%20culm"
                                                    }
                                                ]
                                            },
                                            "text": "tallest culm"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger%20culm"
                                                    }
                                                ]
                                            },
                                            "text": "larger culm"
                                        }
                                    ],
                                    "text": "culm"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flowering-stem"
                                            }
                                        ]
                                    },
                                    "text": "flowering-stem"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#axis"
                                            }
                                        ]
                                    },
                                    "text": "axis"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhizome"
                                            }
                                        ]
                                    },
                                    "text": "rhizome"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stolon"
                                            }
                                        ]
                                    },
                                    "text": "stolon"
                                }
                            ],
                            "text": "stem"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perigynium%20beak"
                                    }
                                ]
                            },
                            "text": "perigynium beak"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#locule"
                                    }
                                ]
                            },
                            "text": "locule"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stamen"
                                    }
                                ]
                            },
                            "text": "stamen"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#faces"
                                    }
                                ]
                            },
                            "text": "faces"
                        }
                    ],
                    "text": "material anatomical entity"
                },
                largeDataQuality: {
                    "data": {
                        "details": [
                            {
                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quality"
                            }
                        ]
                    },
                    "children": [
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#maturity"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mature"
                                            }
                                        ]
                                    },
                                    "text": "mature"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#new"
                                            }
                                        ]
                                    },
                                    "text": "new"
                                }
                            ],
                            "text": "maturity"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shape"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#filiform"
                                            }
                                        ]
                                    },
                                    "text": "filiform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subulate"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lance-subulate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "lance-subulate"
                                                }
                                            ],
                                            "text": "subulate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-linear"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-linear"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-elongate"
                                                    }
                                                ]
                                            },
                                            "text": "linear-elongate"
                                        }
                                    ],
                                    "text": "linear"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dilated"
                                            }
                                        ]
                                    },
                                    "text": "dilated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#trabeculate"
                                            }
                                        ]
                                    },
                                    "text": "trabeculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obtuse",
                                                "exact synonyms": "obtusely"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subobtuse"
                                                    }
                                                ]
                                            },
                                            "text": "subobtuse"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inrolled%20obtuse"
                                                    }
                                                ]
                                            },
                                            "text": "inrolled obtuse"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-obtuse"
                                                    }
                                                ]
                                            },
                                            "text": "rounded-obtuse"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20obtuse"
                                                    }
                                                ]
                                            },
                                            "text": "broadly obtuse"
                                        }
                                    ],
                                    "text": "obtuse"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blunt"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20blunt"
                                                    }
                                                ]
                                            },
                                            "text": "usually blunt"
                                        }
                                    ],
                                    "text": "blunt"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#caudate"
                                            }
                                        ]
                                    },
                                    "text": "caudate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quadrangular"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quadrate"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-quadrate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "ovate-quadrate"
                                                }
                                            ],
                                            "text": "quadrate"
                                        }
                                    ],
                                    "text": "quadrangular"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceolate-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "lanceolate-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovoid-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "ovoid-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elongate-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "elongate-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovateoblong"
                                                    }
                                                ]
                                            },
                                            "text": "ovateoblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "rhombic-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "linear-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "short-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20oblong"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly oblong"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate-oblong"
                                                    }
                                                ]
                                            },
                                            "text": "obovate-oblong"
                                        }
                                    ],
                                    "text": "oblong"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pyriform"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obpyriform"
                                                    }
                                                ]
                                            },
                                            "text": "obpyriform"
                                        }
                                    ],
                                    "text": "pyriform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#truncate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subtruncate"
                                                    }
                                                ]
                                            },
                                            "text": "subtruncate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20truncate"
                                                    }
                                                ]
                                            },
                                            "text": "slightly truncate"
                                        }
                                    ],
                                    "text": "truncate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tubular"
                                            }
                                        ]
                                    },
                                    "text": "tubular"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#auriculate"
                                            }
                                        ]
                                    },
                                    "text": "auriculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orbicular-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "orbicular-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subovate"
                                                    }
                                                ]
                                            },
                                            "text": "subovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "rhombic-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "linear-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#globose-obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "globose-obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "oblong-obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#weakly%20obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "weakly obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "narrowly obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "broadly obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sharply%20obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "sharply obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subobovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "subobovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "elliptic-obovate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#circular-obovate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "circular-obovate"
                                                }
                                            ],
                                            "text": "obovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deltate-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "deltate-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broad-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "broad-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#circular-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "circular-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhomboid-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "rhomboid-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceovate"
                                                    }
                                                ]
                                            },
                                            "text": "lanceovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20ovate"
                                                    }
                                                ]
                                            },
                                            "text": "broadly ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20ovate"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20ovate"
                                                    }
                                                ]
                                            },
                                            "text": "usually ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#trullate-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "trullate-ovate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceolate-ovate"
                                                    }
                                                ]
                                            },
                                            "text": "lanceolate-ovate"
                                        }
                                    ],
                                    "text": "ovate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cucullate"
                                            }
                                        ]
                                    },
                                    "text": "cucullate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#setaceous"
                                            }
                                        ]
                                    },
                                    "text": "setaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acute",
                                                "exact synonyms": "acutely"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nearly%20acute"
                                                    }
                                                ]
                                            },
                                            "text": "nearly acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-acute"
                                                    }
                                                ]
                                            },
                                            "text": "short-acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20acute"
                                                    }
                                                ]
                                            },
                                            "text": "usually acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate%20acute"
                                                    }
                                                ]
                                            },
                                            "text": "ovate acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subacute"
                                                    }
                                                ]
                                            },
                                            "text": "subacute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-acute"
                                                    }
                                                ]
                                            },
                                            "text": "long-acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20acute"
                                                    }
                                                ]
                                            },
                                            "text": "broadly acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#less%20acute"
                                                    }
                                                ]
                                            },
                                            "text": "less acute"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acutish"
                                                    }
                                                ]
                                            },
                                            "text": "acutish"
                                        }
                                    ],
                                    "text": "acute"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rolled"
                                            }
                                        ]
                                    },
                                    "text": "rolled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#entire"
                                            }
                                        ]
                                    },
                                    "text": "entire"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#funnelform"
                                            }
                                        ]
                                    },
                                    "text": "funnelform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inflated"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strongly%20inflated"
                                                    }
                                                ]
                                            },
                                            "text": "strongly inflated"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subinflated"
                                                    }
                                                ]
                                            },
                                            "text": "subinflated"
                                        }
                                    ],
                                    "text": "inflated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sinuate"
                                            }
                                        ]
                                    },
                                    "text": "sinuate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#y-shaped"
                                            }
                                        ]
                                    },
                                    "text": "y-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#triangular"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#triangular-channeled"
                                                    }
                                                ]
                                            },
                                            "text": "triangular-channeled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#trigonous"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-trigonous",
                                                                "exact synonyms": "round-trigonous"
                                                            }
                                                        ]
                                                    },
                                                    "text": "rounded-trigonous"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flattened-trigonous"
                                                            }
                                                        ]
                                                    },
                                                    "text": "flattened-trigonous"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compressed-trigonous"
                                                            }
                                                        ]
                                                    },
                                                    "text": "compressed-trigonous"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sharply%20trigonous"
                                                            }
                                                        ]
                                                    },
                                                    "text": "sharply trigonous"
                                                }
                                            ],
                                            "text": "trigonous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pyramidal"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obpyramidal"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obpyramidal"
                                                }
                                            ],
                                            "text": "pyramidal"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obtriangular"
                                                    }
                                                ]
                                            },
                                            "text": "obtriangular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-triangular"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-triangular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flattened-triangular"
                                                    }
                                                ]
                                            },
                                            "text": "flattened-triangular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#triquetrous"
                                                    }
                                                ]
                                            },
                                            "text": "triquetrous"
                                        }
                                    ],
                                    "text": "triangular"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#aristate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-aristate"
                                                    }
                                                ]
                                            },
                                            "text": "short-aristate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-aristate"
                                                    }
                                                ]
                                            },
                                            "text": "long-aristate"
                                        }
                                    ],
                                    "text": "aristate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#u-shaped"
                                            }
                                        ]
                                    },
                                    "text": "u-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pleated"
                                            }
                                        ]
                                    },
                                    "text": "pleated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#w-shaped"
                                            }
                                        ]
                                    },
                                    "text": "w-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#m-shaped"
                                            }
                                        ]
                                    },
                                    "text": "m-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acuminate",
                                                "elucidation": "https://drive.google.com/open?id=1Gf-k7w2U5OYXg8UwTl7NSNcJOvLq4Aey"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceolate-acuminate"
                                                    }
                                                ]
                                            },
                                            "text": "lanceolate-acuminate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-acuminate"
                                                    }
                                                ]
                                            },
                                            "text": "long-acuminate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-acuminate"
                                                    }
                                                ]
                                            },
                                            "text": "short-acuminate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#suborbicular"
                                                    }
                                                ]
                                            },
                                            "text": "suborbicular"
                                        }
                                    ],
                                    "text": "acuminate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oval"
                                            }
                                        ]
                                    },
                                    "text": "oval"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terete"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#half-terete"
                                                    }
                                                ]
                                            },
                                            "text": "half-terete"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate%20terete"
                                                    }
                                                ]
                                            },
                                            "text": "obovate terete"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subterete"
                                                    }
                                                ]
                                            },
                                            "text": "subterete"
                                        }
                                    ],
                                    "text": "terete"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#revolute"
                                            }
                                        ]
                                    },
                                    "text": "revolute"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#denticulate"
                                            }
                                        ]
                                    },
                                    "text": "denticulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ridged"
                                            }
                                        ]
                                    },
                                    "text": "ridged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceoloid"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblanceoloid"
                                                    }
                                                ]
                                            },
                                            "text": "oblanceoloid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-lanceoloid"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-lanceoloid"
                                        }
                                    ],
                                    "text": "lanceoloid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cylindric",
                                                "exact synonyms": "cylindrical"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "long-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slenderly%20cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "slenderly cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clavate-cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "clavate-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortly%20cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "shortly cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-cylindric",
                                                        "exact synonyms": "oblong-cylindrical"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-cylindrical"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-cylindrical"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20cylindric",
                                                        "exact synonyms": "slenderly cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-cylindric",
                                                        "exact synonyms": "linear-cylindrical"
                                                    }
                                                ]
                                            },
                                            "text": "linear-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short%20cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "short cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#involute-cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "involute-cylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subcylindric"
                                                    }
                                                ]
                                            },
                                            "text": "subcylindric"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-cylindrical"
                                                    }
                                                ]
                                            },
                                            "text": "linear-cylindrical"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-cylindric",
                                                        "exact synonyms": "shortly cylindric"
                                                    }
                                                ]
                                            },
                                            "text": "short-cylindric"
                                        }
                                    ],
                                    "text": "cylindric"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scalloped"
                                            }
                                        ]
                                    },
                                    "text": "scalloped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pointed"
                                            }
                                        ]
                                    },
                                    "text": "pointed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#crinkled"
                                            }
                                        ]
                                    },
                                    "text": "crinkled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cordlike"
                                            }
                                        ]
                                    },
                                    "text": "cordlike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clavate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subclavate"
                                                    }
                                                ]
                                            },
                                            "text": "subclavate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-clavate"
                                                    }
                                                ]
                                            },
                                            "text": "long-clavate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-clavate"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-clavate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-clavate"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-clavate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-clavate"
                                                    }
                                                ]
                                            },
                                            "text": "linear-clavate"
                                        }
                                    ],
                                    "text": "clavate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#outcurved"
                                            }
                                        ]
                                    },
                                    "text": "outcurved"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inrolled"
                                            }
                                        ]
                                    },
                                    "text": "inrolled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#capitate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subcapitate"
                                                    }
                                                ]
                                            },
                                            "text": "subcapitate"
                                        }
                                    ],
                                    "text": "capitate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#diamond-shaped"
                                            }
                                        ]
                                    },
                                    "text": "diamond-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strap-shaped"
                                            }
                                        ]
                                    },
                                    "text": "strap-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#curled"
                                            }
                                        ]
                                    },
                                    "text": "curled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tapered",
                                                "exact synonyms": "tapering"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long%20tapered"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-tapered"
                                                            }
                                                        ]
                                                    },
                                                    "text": "long-tapered"
                                                }
                                            ],
                                            "text": "long tapered"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-tapered"
                                                    }
                                                ]
                                            },
                                            "text": "short-tapered"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-tapering",
                                                        "exact synonyms": "long tapered"
                                                    }
                                                ]
                                            },
                                            "text": "long-tapering"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gradually%20tapering",
                                                        "exact synonyms": "gradually tapered"
                                                    }
                                                ]
                                            },
                                            "text": "gradually tapering"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-tapering",
                                                        "exact synonyms": "round-tapering"
                                                    }
                                                ]
                                            },
                                            "text": "rounded-tapering"
                                        }
                                    ],
                                    "text": "tapered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orbiculate",
                                                "exact synonyms": "orbicular"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate-orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "obovate-orbicular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-orbicular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nearly%20orbiculate",
                                                        "exact synonyms": "nearly orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "nearly orbiculate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovoid-orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "ovoid-orbicular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic-orbiculate",
                                                        "exact synonyms": "rhombic-orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "rhombic-orbiculate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-orbiculate",
                                                        "exact synonyms": "ovate-orbicular"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-orbiculate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#suborbiculate",
                                                        "exact synonyms": "suborbicular"
                                                    }
                                                ]
                                            },
                                            "text": "suborbiculate"
                                        }
                                    ],
                                    "text": "orbiculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#irregular",
                                                "exact synonyms": "irregularly"
                                            }
                                        ]
                                    },
                                    "text": "irregular"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deltate"
                                            }
                                        ]
                                    },
                                    "text": "deltate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cuspidate",
                                                "exact synonyms": "cuspitate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-cuspidate"
                                                    }
                                                ]
                                            },
                                            "text": "short-cuspidate"
                                        }
                                    ],
                                    "text": "cuspidate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#retuse"
                                            }
                                        ]
                                    },
                                    "text": "retuse"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mucronate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-mucronate"
                                                    }
                                                ]
                                            },
                                            "text": "short-mucronate"
                                        }
                                    ],
                                    "text": "mucronate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compressed"
                                            }
                                        ]
                                    },
                                    "text": "compressed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#knobbed"
                                            }
                                        ]
                                    },
                                    "text": "knobbed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fibriform"
                                            }
                                        ]
                                    },
                                    "text": "fibriform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#needle-shaped"
                                            }
                                        ]
                                    },
                                    "text": "needle-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flared"
                                            }
                                        ]
                                    },
                                    "text": "flared"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#v-shaped"
                                            }
                                        ]
                                    },
                                    "text": "v-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gutter--shaped"
                                            }
                                        ]
                                    },
                                    "text": "gutter--shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceolate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblanceolate"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-oblanceolate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "elliptic-oblanceolate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-oblanceolate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "oblong-oblanceolate"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20oblanceloid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "narrowly oblanceloid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovoid-oblanceoloid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obovoid-oblanceoloid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-oblanceolate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "linear-oblanceolate"
                                                }
                                            ],
                                            "text": "oblanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "broadly lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate-lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "obovate-lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#linear-lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "linear-lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sublanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "sublanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-lanceolate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20lanceolate"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly lanceolate"
                                        }
                                    ],
                                    "text": "lanceolate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cuneate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20cuneate"
                                                    }
                                                ]
                                            },
                                            "text": "broadly cuneate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-cuneate"
                                                    }
                                                ]
                                            },
                                            "text": "rounded-cuneate"
                                        }
                                    ],
                                    "text": "cuneate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#toothed"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#minutely%20toothed"
                                                    }
                                                ]
                                            },
                                            "text": "minutely toothed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bidentate"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortly%20bidentate"
                                                            }
                                                        ]
                                                    },
                                                    "text": "shortly bidentate"
                                                }
                                            ],
                                            "text": "bidentate"
                                        }
                                    ],
                                    "text": "toothed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scale-shaped"
                                            }
                                        ]
                                    },
                                    "text": "scale-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awl-shaped"
                                            }
                                        ]
                                    },
                                    "text": "awl-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awn-shaped",
                                                "exact synonyms": "awnlike"
                                            }
                                        ]
                                    },
                                    "text": "awn-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#involute"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#filiform-involute"
                                                    }
                                                ]
                                            },
                                            "text": "filiform-involute"
                                        }
                                    ],
                                    "text": "involute"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sharp"
                                            }
                                        ]
                                    },
                                    "text": "sharp"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hemispheric"
                                            }
                                        ]
                                    },
                                    "text": "hemispheric"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#crenulate"
                                            }
                                        ]
                                    },
                                    "text": "crenulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cordate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subcordate"
                                                    }
                                                ]
                                            },
                                            "text": "subcordate"
                                        }
                                    ],
                                    "text": "cordate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flat",
                                                "exact synonyms": "flattish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovoid%20flat"
                                                    }
                                                ]
                                            },
                                            "text": "obovoid flat"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orbiculate%20flat"
                                                    }
                                                ]
                                            },
                                            "text": "orbiculate flat"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subflat"
                                                    }
                                                ]
                                            },
                                            "text": "subflat"
                                        }
                                    ],
                                    "text": "flat"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#convolute"
                                            }
                                        ]
                                    },
                                    "text": "convolute"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hair-shaped"
                                            }
                                        ]
                                    },
                                    "text": "hair-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apiculate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-apiculate"
                                                    }
                                                ]
                                            },
                                            "text": "short-apiculate"
                                        }
                                    ],
                                    "text": "apiculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowed"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abruptly%20narrowed"
                                                    }
                                                ]
                                            },
                                            "text": "abruptly narrowed"
                                        }
                                    ],
                                    "text": "narrowed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fusiform"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-fusiform"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-fusiform"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-fusiform"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-fusiform"
                                        }
                                    ],
                                    "text": "fusiform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#undulate"
                                            }
                                        ]
                                    },
                                    "text": "undulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#symmetric"
                                            }
                                        ]
                                    },
                                    "text": "symmetric"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#angled"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sharp-angled"
                                                    }
                                                ]
                                            },
                                            "text": "sharp-angled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-angled"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-angled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smooth-angled"
                                                    }
                                                ]
                                            },
                                            "text": "smooth-angled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-angled"
                                                    }
                                                ]
                                            },
                                            "text": "rounded-angled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wing-angled"
                                                    }
                                                ]
                                            },
                                            "text": "wing-angled"
                                        }
                                    ],
                                    "text": "angled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#boatshaped"
                                            }
                                        ]
                                    },
                                    "text": "boatshaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reniform"
                                            }
                                        ]
                                    },
                                    "text": "reniform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hood--shaped"
                                            }
                                        ]
                                    },
                                    "text": "hood--shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#conic",
                                                "exact synonyms": "conically"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obconic"
                                                    }
                                                ]
                                            },
                                            "text": "obconic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20conic"
                                                    }
                                                ]
                                            },
                                            "text": "slightly conic"
                                        }
                                    ],
                                    "text": "conic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#uninflated"
                                            }
                                        ]
                                    },
                                    "text": "uninflated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#asymmetric"
                                            }
                                        ]
                                    },
                                    "text": "asymmetric"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#concave"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20concave"
                                                    }
                                                ]
                                            },
                                            "text": "slightly concave"
                                        }
                                    ],
                                    "text": "concave"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#circular"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subcircular"
                                                            }
                                                        ]
                                                    },
                                                    "text": "subcircular"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-circular"
                                                            }
                                                        ]
                                                    },
                                                    "text": "ovate-circular"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate-circular"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obovate-circular"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-circular"
                                                            }
                                                        ]
                                                    },
                                                    "text": "elliptic-circular"
                                                }
                                            ],
                                            "text": "circular"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovate-elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "obovate-elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblanceolate-elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "oblanceolate-elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceolate-elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "lanceolate-elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "broadly elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded",
                                                        "exact synonyms": "round"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovoid%20round"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obovoid round"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subrounded"
                                                            }
                                                        ]
                                                    },
                                                    "text": "subrounded"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acute-rounded"
                                                            }
                                                        ]
                                                    },
                                                    "text": "acute-rounded"
                                                }
                                            ],
                                            "text": "rounded"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-elliptic"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ellipsoid"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "narrowly ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "broadly ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovoid-ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obovoid-ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lance-ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "lance-ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widely%20ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "widely ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "oblong-ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic-ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "rhombic-ellipsoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovoid-ellipsoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "ovoid-ellipsoid"
                                                }
                                            ],
                                            "text": "ellipsoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20elliptic",
                                                        "exact synonyms": "narrowly ellipsoid"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly elliptic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subelliptic"
                                                    }
                                                ]
                                            },
                                            "text": "subelliptic"
                                        }
                                    ],
                                    "text": "elliptic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#falcate"
                                            }
                                        ]
                                    },
                                    "text": "falcate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#globose"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovoid-globose"
                                                    }
                                                ]
                                            },
                                            "text": "ovoid-globose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-globose"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-globose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subglobose"
                                                    }
                                                ]
                                            },
                                            "text": "subglobose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#globular"
                                                    }
                                                ]
                                            },
                                            "text": "globular"
                                        }
                                    ],
                                    "text": "globose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#indented"
                                            }
                                        ]
                                    },
                                    "text": "indented"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic",
                                                "exact synonyms": "rhomboid"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-rhombic"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-rhombic"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovate-rhomboid"
                                                    }
                                                ]
                                            },
                                            "text": "ovate-rhomboid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subrhombic"
                                                    }
                                                ]
                                            },
                                            "text": "subrhombic"
                                        }
                                    ],
                                    "text": "rhombic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distended"
                                            }
                                        ]
                                    },
                                    "text": "distended"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spathose-shaped"
                                            }
                                        ]
                                    },
                                    "text": "spathose-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quill-shaped"
                                            }
                                        ]
                                    },
                                    "text": "quill-shaped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flatly"
                                            }
                                        ]
                                    },
                                    "text": "flatly"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#attenuate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-attenuate"
                                                    }
                                                ]
                                            },
                                            "text": "short-attenuate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cuneate-attenuate"
                                                    }
                                                ]
                                            },
                                            "text": "cuneate-attenuate"
                                        }
                                    ],
                                    "text": "attenuate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#utriculiform"
                                            }
                                        ]
                                    },
                                    "text": "utriculiform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#emarginate"
                                            }
                                        ]
                                    },
                                    "text": "emarginate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#circinnate"
                                            }
                                        ]
                                    },
                                    "text": "circinnate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ovoid"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "usually ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "short-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deltoid-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "deltoid-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhomboid-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "rhomboid-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "narrowly ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "broadly ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clavate-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "clavate-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elliptic-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "elliptic-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obovoid"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obdeltoid-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "obdeltoid-obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compressed-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "compressed-obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elongate-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "elongate-obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadly%20obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "broadly obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ellipsoid-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "ellipsoid-obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "oblong-obovoid"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#trigonous-obovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "trigonous-obovoid"
                                                }
                                            ],
                                            "text": "obovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#globose-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "globose-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceovoid"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowly%20lanceovoid"
                                                            }
                                                        ]
                                                    },
                                                    "text": "narrowly lanceovoid"
                                                }
                                            ],
                                            "text": "lanceovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compressed-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "compressed-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ellipsoid-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "ellipsoid-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhombic-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "rhombic-ovoid"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblong-ovoid"
                                                    }
                                                ]
                                            },
                                            "text": "oblong-ovoid"
                                        }
                                    ],
                                    "text": "ovoid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#convex",
                                                "exact synonyms": "convexly"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#biconvex"
                                                    }
                                                ]
                                            },
                                            "text": "biconvex"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#plano-convex",
                                                        "exact synonyms": "planoconvex"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subplano-convex"
                                                            }
                                                        ]
                                                    },
                                                    "text": "subplano-convex"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lanceovate%20planoconvex"
                                                            }
                                                        ]
                                                    },
                                                    "text": "lanceovate planoconvex"
                                                }
                                            ],
                                            "text": "plano-convex"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20convex"
                                                    }
                                                ]
                                            },
                                            "text": "slightly convex"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-convex"
                                                    }
                                                ]
                                            },
                                            "text": "short-convex"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#concavo-convex"
                                                    }
                                                ]
                                            },
                                            "text": "concavo-convex"
                                        }
                                    ],
                                    "text": "convex"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nobbed"
                                            }
                                        ]
                                    },
                                    "text": "nobbed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spherical"
                                            }
                                        ]
                                    },
                                    "text": "spherical"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#serrulate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ciliate-serrulate"
                                                    }
                                                ]
                                            },
                                            "text": "ciliate-serrulate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#setulose-serrulate"
                                                    }
                                                ]
                                            },
                                            "text": "setulose-serrulate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subserrulate"
                                                    }
                                                ]
                                            },
                                            "text": "subserrulate"
                                        }
                                    ],
                                    "text": "serrulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#annulate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#discoid-annulate"
                                                    }
                                                ]
                                            },
                                            "text": "discoid-annulate"
                                        }
                                    ],
                                    "text": "annulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mitrate",
                                                "exact synonyms": "miter-shaped"
                                            }
                                        ]
                                    },
                                    "text": "mitrate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beaked"
                                            }
                                        ]
                                    },
                                    "text": "beaked"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bulging"
                                            }
                                        ]
                                    },
                                    "text": "bulging"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tattered"
                                            }
                                        ]
                                    },
                                    "text": "tattered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#channeled"
                                            }
                                        ]
                                    },
                                    "text": "channeled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#notched"
                                            }
                                        ]
                                    },
                                    "text": "notched"
                                }
                            ],
                            "text": "shape"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orientation"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deflexed"
                                            }
                                        ]
                                    },
                                    "text": "deflexed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#appressed"
                                            }
                                        ]
                                    },
                                    "text": "appressed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reflexed"
                                            }
                                        ]
                                    },
                                    "text": "reflexed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#recurved"
                                            }
                                        ]
                                    },
                                    "text": "recurved"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#incurved"
                                            }
                                        ]
                                    },
                                    "text": "incurved"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nodding"
                                            }
                                        ]
                                    },
                                    "text": "nodding"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#erect"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#appressed-erect"
                                                    }
                                                ]
                                            },
                                            "text": "appressed-erect"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#suberect"
                                                    }
                                                ]
                                            },
                                            "text": "suberect"
                                        }
                                    ],
                                    "text": "erect"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#arising"
                                            }
                                        ]
                                    },
                                    "text": "arising"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inclined"
                                            }
                                        ]
                                    },
                                    "text": "inclined"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ascending"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#erect-ascending"
                                                    }
                                                ]
                                            },
                                            "text": "erect-ascending"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#appressed-ascending"
                                                    }
                                                ]
                                            },
                                            "text": "appressed-ascending"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spreading-ascending"
                                                    }
                                                ]
                                            },
                                            "text": "spreading-ascending"
                                        }
                                    ],
                                    "text": "ascending"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#arching"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20arching"
                                                    }
                                                ]
                                            },
                                            "text": "slightly arching"
                                        }
                                    ],
                                    "text": "arching"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pendent"
                                            }
                                        ]
                                    },
                                    "text": "pendent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-arching"
                                            }
                                        ]
                                    },
                                    "text": "long-arching"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spreading"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#loosely%20spreading"
                                                    }
                                                ]
                                            },
                                            "text": "loosely spreading"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#weakly%20spreading"
                                                    }
                                                ]
                                            },
                                            "text": "weakly spreading"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#horizontally%20spreading"
                                                    }
                                                ]
                                            },
                                            "text": "horizontally spreading"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widely%20spreading"
                                                    }
                                                ]
                                            },
                                            "text": "widely spreading"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ascending-spreading"
                                                    }
                                                ]
                                            },
                                            "text": "ascending-spreading"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#squarrose-spreading"
                                                    }
                                                ]
                                            },
                                            "text": "squarrose-spreading"
                                        }
                                    ],
                                    "text": "spreading"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#drooping"
                                            }
                                        ]
                                    },
                                    "text": "drooping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#horizontal"
                                            }
                                        ]
                                    },
                                    "text": "horizontal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#projecting"
                                            }
                                        ]
                                    },
                                    "text": "projecting"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#oblique"
                                            }
                                        ]
                                    },
                                    "text": "oblique"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#recurrent"
                                            }
                                        ]
                                    },
                                    "text": "recurrent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reclining"
                                            }
                                        ]
                                    },
                                    "text": "reclining"
                                }
                            ],
                            "text": "orientation"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#color%20brightness"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#high%20brightness",
                                                "exact synonyms": "light"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20greenish"
                                                    }
                                                ]
                                            },
                                            "text": "pale greenish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20greenish",
                                                        "exact synonyms": "pale greenish"
                                                    }
                                                ]
                                            },
                                            "text": "light greenish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light-redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "light red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright%20green"
                                                    }
                                                ]
                                            },
                                            "text": "bright green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20ferrugineous"
                                                    }
                                                ]
                                            },
                                            "text": "pale ferrugineous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20green"
                                                    }
                                                ]
                                            },
                                            "text": "pale green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20whitish"
                                                    }
                                                ]
                                            },
                                            "text": "pale whitish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "pale red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "pale yellowish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright-yellow"
                                                    }
                                                ]
                                            },
                                            "text": "bright-yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20pale"
                                                    }
                                                ]
                                            },
                                            "text": "tinged pale"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright%20orange"
                                                    }
                                                ]
                                            },
                                            "text": "bright orange"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale-yellow",
                                                        "exact synonyms": "pale yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "pale-yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light-brown",
                                                        "exact synonyms": "light brown"
                                                    }
                                                ]
                                            },
                                            "text": "light brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20colored"
                                                    }
                                                ]
                                            },
                                            "text": "light colored"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale",
                                                        "exact synonyms": "lighter"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20pale"
                                                            }
                                                        ]
                                                    },
                                                    "text": "silvery pale"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20bright"
                                                            }
                                                        ]
                                                    },
                                                    "text": "pale bright"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#paler"
                                                            }
                                                        ]
                                                    },
                                                    "text": "paler"
                                                }
                                            ],
                                            "text": "pale"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20silvery"
                                                    }
                                                ]
                                            },
                                            "text": "pale silvery"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lighter%20colored"
                                                    }
                                                ]
                                            },
                                            "text": "lighter colored"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "pale hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20green",
                                                        "exact synonyms": "pale greenish"
                                                    }
                                                ]
                                            },
                                            "text": "light green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20olive",
                                                        "exact synonyms": "light olive"
                                                    }
                                                ]
                                            },
                                            "text": "pale olive"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20brownish"
                                                    }
                                                ]
                                            },
                                            "text": "pale brownish"
                                        }
                                    ],
                                    "text": "high brightness"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#low%20brightness",
                                                "exact synonyms": "dark"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20purple"
                                                    }
                                                ]
                                            },
                                            "text": "dark purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark-brown",
                                                        "exact synonyms": "dark-brown"
                                                    }
                                                ]
                                            },
                                            "text": "dark brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "dark tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark-colored"
                                                    }
                                                ]
                                            },
                                            "text": "dark-colored"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20purplish"
                                                    }
                                                ]
                                            },
                                            "text": "dark purplish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20maroon"
                                                    }
                                                ]
                                            },
                                            "text": "dark maroon"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20reddish"
                                                    }
                                                ]
                                            },
                                            "text": "dark reddish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20redbrown",
                                                        "exact synonyms": "dark redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "dark red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#darkened"
                                                    }
                                                ]
                                            },
                                            "text": "darkened"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20green"
                                                    }
                                                ]
                                            },
                                            "text": "dark green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20red"
                                                    }
                                                ]
                                            },
                                            "text": "dark red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark",
                                                        "exact synonyms": "dark-colored"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dusky"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dusky"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#darker"
                                                            }
                                                        ]
                                                    },
                                                    "text": "darker"
                                                }
                                            ],
                                            "text": "dark"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20olive"
                                                    }
                                                ]
                                            },
                                            "text": "dark olive"
                                        }
                                    ],
                                    "text": "low brightness"
                                }
                            ],
                            "text": "color brightness"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reflection"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#translucent"
                                            }
                                        ]
                                    },
                                    "text": "translucent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright"
                                            }
                                        ]
                                    },
                                    "text": "bright"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#opaque"
                                            }
                                        ]
                                    },
                                    "text": "opaque"
                                }
                            ],
                            "text": "reflection"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#texture"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#membranaceous"
                                            }
                                        ]
                                    },
                                    "text": "membranaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rough",
                                                "exact synonyms": "roughened"
                                            }
                                        ]
                                    },
                                    "text": "rough"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hard"
                                            }
                                        ]
                                    },
                                    "text": "hard"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coriaceous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subcoriaceous"
                                                    }
                                                ]
                                            },
                                            "text": "subcoriaceous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thick-coriaceous"
                                                    }
                                                ]
                                            },
                                            "text": "thick-coriaceous"
                                        }
                                    ],
                                    "text": "coriaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cartilaginous"
                                            }
                                        ]
                                    },
                                    "text": "cartilaginous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#membranous",
                                                "exact synonyms": "membranaceous"
                                            }
                                        ]
                                    },
                                    "text": "membranous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#papery",
                                                "exact synonyms": "chartaceous"
                                            }
                                        ]
                                    },
                                    "text": "papery"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coarse",
                                                "exact synonyms": "coarse-textured"
                                            }
                                        ]
                                    },
                                    "text": "coarse"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fleshy"
                                            }
                                        ]
                                    },
                                    "text": "fleshy"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scarious"
                                            }
                                        ]
                                    },
                                    "text": "scarious"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fine-textured"
                                            }
                                        ]
                                    },
                                    "text": "fine-textured"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#friable"
                                            }
                                        ]
                                    },
                                    "text": "friable"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#corky"
                                            }
                                        ]
                                    },
                                    "text": "corky"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fibrillose"
                                            }
                                        ]
                                    },
                                    "text": "fibrillose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#firm"
                                            }
                                        ]
                                    },
                                    "text": "firm"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coarse-textured"
                                            }
                                        ]
                                    },
                                    "text": "coarse-textured"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spongy"
                                            }
                                        ]
                                    },
                                    "text": "spongy"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#soft"
                                            }
                                        ]
                                    },
                                    "text": "soft"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rigid"
                                            }
                                        ]
                                    },
                                    "text": "rigid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leathery"
                                            }
                                        ]
                                    },
                                    "text": "leathery"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fibrous"
                                            }
                                        ]
                                    },
                                    "text": "fibrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flaccid"
                                            }
                                        ]
                                    },
                                    "text": "flaccid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#chartaceous"
                                            }
                                        ]
                                    },
                                    "text": "chartaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#roughened"
                                            }
                                        ]
                                    },
                                    "text": "roughened"
                                }
                            ],
                            "text": "texture"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pubescence"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#setose"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortly%20setose"
                                                    }
                                                ]
                                            },
                                            "text": "shortly setose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ciliate-setose"
                                                    }
                                                ]
                                            },
                                            "text": "ciliate-setose"
                                        }
                                    ],
                                    "text": "setose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smooth"
                                            }
                                        ]
                                    },
                                    "text": "smooth"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glabrescent"
                                            }
                                        ]
                                    },
                                    "text": "glabrescent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hispidulous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rough-hispidulous"
                                                    }
                                                ]
                                            },
                                            "text": "rough-hispidulous"
                                        }
                                    ],
                                    "text": "hispidulous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pubescent"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "short-pubescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#appressed-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "appressed-pubescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "long-pubescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stellate-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "stellate-pubescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spreading-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "spreading-pubescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-pubescent"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-pubescent"
                                        }
                                    ],
                                    "text": "pubescent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ciliate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "long-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#setose-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "setose-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rough-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "rough-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#erose-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "erose-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "short-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-ciliate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#serrulate-ciliate"
                                                    }
                                                ]
                                            },
                                            "text": "serrulate-ciliate"
                                        }
                                    ],
                                    "text": "ciliate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hispid"
                                            }
                                        ]
                                    },
                                    "text": "hispid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hirsute"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hirtellous"
                                                    }
                                                ]
                                            },
                                            "text": "hirtellous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stellate-hirsute"
                                                    }
                                                ]
                                            },
                                            "text": "stellate-hirsute"
                                        }
                                    ],
                                    "text": "hirsute"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strigose"
                                            }
                                        ]
                                    },
                                    "text": "strigose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bristly"
                                            }
                                        ]
                                    },
                                    "text": "bristly"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#puberulent"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#puberulent%20proximally"
                                                    }
                                                ]
                                            },
                                            "text": "puberulent proximally"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-puberulent"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-puberulent"
                                        }
                                    ],
                                    "text": "puberulent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pilose"
                                            }
                                        ]
                                    },
                                    "text": "pilose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#felty"
                                            }
                                        ]
                                    },
                                    "text": "felty"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glaucous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glaucous-green",
                                                        "exact synonyms": "glaucous-green"
                                                    }
                                                ]
                                            },
                                            "text": "glaucous green"
                                        }
                                    ],
                                    "text": "glaucous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glabrous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subglabrous"
                                                    }
                                                ]
                                            },
                                            "text": "subglabrous"
                                        }
                                    ],
                                    "text": "glabrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hairy"
                                            }
                                        ]
                                    },
                                    "text": "hairy"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scaly"
                                            }
                                        ]
                                    },
                                    "text": "scaly"
                                }
                            ],
                            "text": "pubescence"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reflectance"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shiny",
                                                "exact synonyms": "shining"
                                            }
                                        ]
                                    },
                                    "text": "shiny"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20shiny"
                                            }
                                        ]
                                    },
                                    "text": "slightly shiny"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shining"
                                            }
                                        ]
                                    },
                                    "text": "shining"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glossy"
                                            }
                                        ]
                                    },
                                    "text": "glossy"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#satiny"
                                            }
                                        ]
                                    },
                                    "text": "satiny"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20glossy"
                                            }
                                        ]
                                    },
                                    "text": "slightly glossy"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheen"
                                            }
                                        ]
                                    },
                                    "text": "sheen"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pearlescent"
                                            }
                                        ]
                                    },
                                    "text": "pearlescent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lustrous"
                                            }
                                        ]
                                    },
                                    "text": "lustrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dull"
                                            }
                                        ]
                                    },
                                    "text": "dull"
                                }
                            ],
                            "text": "reflectance"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quantity",
                                        "exact synonyms": "count"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#many",
                                                "exact synonyms": "numerous"
                                            }
                                        ]
                                    },
                                    "text": "many"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rare"
                                            }
                                        ]
                                    },
                                    "text": "rare"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#few"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fewer"
                                                    }
                                                ]
                                            },
                                            "text": "fewer"
                                        }
                                    ],
                                    "text": "few"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nearly"
                                            }
                                        ]
                                    },
                                    "text": "nearly"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#numerous"
                                            }
                                        ]
                                    },
                                    "text": "numerous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abundant"
                                            }
                                        ]
                                    },
                                    "text": "abundant"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lacking"
                                            }
                                        ]
                                    },
                                    "text": "lacking"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#several"
                                            }
                                        ]
                                    },
                                    "text": "several"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#single",
                                                "exact synonyms": "singly"
                                            }
                                        ]
                                    },
                                    "text": "single"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#much"
                                            }
                                        ]
                                    },
                                    "text": "much"
                                }
                            ],
                            "text": "quantity"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#depth",
                                        "exact synonyms": "thickness"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deep"
                                            }
                                        ]
                                    },
                                    "text": "deep"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thick"
                                            }
                                        ]
                                    },
                                    "text": "thick"
                                }
                            ],
                            "text": "depth"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#length"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#not%20longer"
                                            }
                                        ]
                                    },
                                    "text": "not longer"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prolonged"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bristlelike-prolonged"
                                                    }
                                                ]
                                            },
                                            "text": "bristlelike-prolonged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-prolonged"
                                                    }
                                                ]
                                            },
                                            "text": "short-prolonged"
                                        }
                                    ],
                                    "text": "prolonged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elongated"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elongating"
                                                    }
                                                ]
                                            },
                                            "text": "elongating"
                                        }
                                    ],
                                    "text": "elongated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20longer"
                                                    }
                                                ]
                                            },
                                            "text": "slightly longer"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longer"
                                                    }
                                                ]
                                            },
                                            "text": "longer"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#much%20longer"
                                                    }
                                                ]
                                            },
                                            "text": "much longer"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#somewhat%20longer"
                                                    }
                                                ]
                                            },
                                            "text": "somewhat longer"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest"
                                                    }
                                                ]
                                            },
                                            "text": "longest"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20longer"
                                                    }
                                                ]
                                            },
                                            "text": "usually longer"
                                        }
                                    ],
                                    "text": "long"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#much%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "much shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distinctly%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "distinctly shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortened"
                                                    }
                                                ]
                                            },
                                            "text": "shortened"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#often%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "often shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#conspicuously%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "conspicuously shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "slightly shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "usually shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortest"
                                                    }
                                                ]
                                            },
                                            "text": "shortest"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mostly%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "mostly shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shorter"
                                                    }
                                                ]
                                            },
                                            "text": "shorter"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clearly%20shorter"
                                                    }
                                                ]
                                            },
                                            "text": "clearly shorter"
                                        }
                                    ],
                                    "text": "short"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#full"
                                            }
                                        ]
                                    },
                                    "text": "full"
                                }
                            ],
                            "text": "length"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#development"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abortive"
                                            }
                                        ]
                                    },
                                    "text": "abortive"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#well-developed"
                                            }
                                        ]
                                    },
                                    "text": "well-developed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rudimentary"
                                            }
                                        ]
                                    },
                                    "text": "rudimentary"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#developed"
                                            }
                                        ]
                                    },
                                    "text": "developed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#modified"
                                            }
                                        ]
                                    },
                                    "text": "modified"
                                }
                            ],
                            "text": "development"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#attachment"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stalked"
                                            }
                                        ]
                                    },
                                    "text": "stalked"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sessile"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subsessile"
                                                    }
                                                ]
                                            },
                                            "text": "subsessile"
                                        }
                                    ],
                                    "text": "sessile"
                                }
                            ],
                            "text": "attachment"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#arrangement"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fascicled"
                                            }
                                        ]
                                    },
                                    "text": "fascicled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#congregated"
                                            }
                                        ]
                                    },
                                    "text": "congregated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ternate"
                                            }
                                        ]
                                    },
                                    "text": "ternate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#separated"
                                            }
                                        ]
                                    },
                                    "text": "separated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#confluent"
                                            }
                                        ]
                                    },
                                    "text": "confluent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adjacent"
                                            }
                                        ]
                                    },
                                    "text": "adjacent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clustered"
                                            }
                                        ]
                                    },
                                    "text": "clustered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#aggregated"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#densely%20aggregated"
                                                    }
                                                ]
                                            },
                                            "text": "densely aggregated"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#loosely%20aggregated"
                                                    }
                                                ]
                                            },
                                            "text": "loosely aggregated"
                                        }
                                    ],
                                    "text": "aggregated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#alternate"
                                            }
                                        ]
                                    },
                                    "text": "alternate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distichous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subdistichous"
                                                    }
                                                ]
                                            },
                                            "text": "subdistichous"
                                        }
                                    ],
                                    "text": "distichous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#multiseriate"
                                            }
                                        ]
                                    },
                                    "text": "multiseriate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#interspersed"
                                            }
                                        ]
                                    },
                                    "text": "interspersed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#corrugate"
                                            }
                                        ]
                                    },
                                    "text": "corrugate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fasciculate",
                                                "exact synonyms": "fascicled"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subfasciculate"
                                                    }
                                                ]
                                            },
                                            "text": "subfasciculate"
                                        }
                                    ],
                                    "text": "fasciculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#paniculate"
                                            }
                                        ]
                                    },
                                    "text": "paniculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#parallel"
                                            }
                                        ]
                                    },
                                    "text": "parallel"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#disposed"
                                            }
                                        ]
                                    },
                                    "text": "disposed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apart"
                                            }
                                        ]
                                    },
                                    "text": "apart"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scattered"
                                            }
                                        ]
                                    },
                                    "text": "scattered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#solitary"
                                            }
                                        ]
                                    },
                                    "text": "solitary"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#singular"
                                            }
                                        ]
                                    },
                                    "text": "singular"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nonoverlapping"
                                            }
                                        ]
                                    },
                                    "text": "nonoverlapping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#overlapping"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#loosely%20overlapping"
                                                    }
                                                ]
                                            },
                                            "text": "loosely overlapping"
                                        }
                                    ],
                                    "text": "overlapping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#corymbose"
                                            }
                                        ]
                                    },
                                    "text": "corymbose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#contiguous"
                                            }
                                        ]
                                    },
                                    "text": "contiguous"
                                }
                            ],
                            "text": "arrangement"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#duration"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#marcescent"
                                            }
                                        ]
                                    },
                                    "text": "marcescent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#persistent",
                                                "exact synonyms": "persisting"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-persistent"
                                                    }
                                                ]
                                            },
                                            "text": "short-persistent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-persistent"
                                                    }
                                                ]
                                            },
                                            "text": "long-persistent"
                                        }
                                    ],
                                    "text": "persistent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#evergreen"
                                            }
                                        ]
                                    },
                                    "text": "evergreen"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#overwintering"
                                            }
                                        ]
                                    },
                                    "text": "overwintering"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#annual"
                                            }
                                        ]
                                    },
                                    "text": "annual"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#late"
                                            }
                                        ]
                                    },
                                    "text": "late"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#deciduous"
                                            }
                                        ]
                                    },
                                    "text": "deciduous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#perennial"
                                            }
                                        ]
                                    },
                                    "text": "perennial"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#early"
                                            }
                                        ]
                                    },
                                    "text": "early"
                                }
                            ],
                            "text": "duration"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#relief"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#punctate"
                                            }
                                        ]
                                    },
                                    "text": "punctate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrid"
                                            }
                                        ]
                                    },
                                    "text": "scabrid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#papillate"
                                            }
                                        ]
                                    },
                                    "text": "papillate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous",
                                                "exact synonyms": "scabrid"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strongly%20scabrous"
                                                    }
                                                ]
                                            },
                                            "text": "strongly scabrous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scaberulous"
                                                    }
                                                ]
                                            },
                                            "text": "scaberulous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#retrorse-scabrous"
                                                    }
                                                ]
                                            },
                                            "text": "retrorse-scabrous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous%20abaxially"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous abaxially"
                                        }
                                    ],
                                    "text": "scabrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#puncticulate"
                                            }
                                        ]
                                    },
                                    "text": "puncticulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#puckered"
                                            }
                                        ]
                                    },
                                    "text": "puckered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#corrugated",
                                                "exact synonyms": "corrugate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cross-corrugated",
                                                        "exact synonyms": "cross-corrugate"
                                                    }
                                                ]
                                            },
                                            "text": "cross-corrugated"
                                        }
                                    ],
                                    "text": "corrugated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pustulate"
                                            }
                                        ]
                                    },
                                    "text": "pustulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unwrinkled"
                                            }
                                        ]
                                    },
                                    "text": "unwrinkled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cross-corrugate"
                                            }
                                        ]
                                    },
                                    "text": "cross-corrugate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#striate"
                                            }
                                        ]
                                    },
                                    "text": "striate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#canaliculate"
                                            }
                                        ]
                                    },
                                    "text": "canaliculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grooved"
                                            }
                                        ]
                                    },
                                    "text": "grooved"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#papillose",
                                                "exact synonyms": "papillate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subpapillose"
                                                    }
                                                ]
                                            },
                                            "text": "subpapillose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red-papillose"
                                                    }
                                                ]
                                            },
                                            "text": "red-papillose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#puberulent-papillose"
                                                    }
                                                ]
                                            },
                                            "text": "puberulent-papillose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#papillose%20abaxially"
                                                    }
                                                ]
                                            },
                                            "text": "papillose abaxially"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#granular-papillose"
                                                    }
                                                ]
                                            },
                                            "text": "granular-papillose"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-papillose"
                                                    }
                                                ]
                                            },
                                            "text": "short-papillose"
                                        }
                                    ],
                                    "text": "papillose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#papilliferous"
                                            }
                                        ]
                                    },
                                    "text": "papilliferous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#erose"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#truncate-erose"
                                                    }
                                                ]
                                            },
                                            "text": "truncate-erose"
                                        }
                                    ],
                                    "text": "erose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rugose"
                                            }
                                        ]
                                    },
                                    "text": "rugose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wrinkled",
                                                "exact synonyms": "rugose"
                                            }
                                        ]
                                    },
                                    "text": "wrinkled"
                                }
                            ],
                            "text": "relief"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#luminous%20flux"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline",
                                                "exact synonyms": "hyaline suffused"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "gold-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tan-hyaline",
                                                        "exact synonyms": "tan-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "tan hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "white-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20golden"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline golden"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "whitish-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white%20hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "white hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20suffused"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline suffused"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20brown",
                                                        "exact synonyms": "brown hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish%20hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "greenish hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrow-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "narrow-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "pale hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20hyaline",
                                                        "exact synonyms": "white hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "whitish hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20hyaline",
                                                        "exact synonyms": "hyaline brown"
                                                    }
                                                ]
                                            },
                                            "text": "brownish hyaline"
                                        }
                                    ],
                                    "text": "hyaline"
                                }
                            ],
                            "text": "luminous flux"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#course"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flexuous"
                                            }
                                        ]
                                    },
                                    "text": "flexuous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bent"
                                            }
                                        ]
                                    },
                                    "text": "bent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#curved",
                                                "exact synonyms": "curving"
                                            }
                                        ]
                                    },
                                    "text": "curved"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#straight"
                                            }
                                        ]
                                    },
                                    "text": "straight"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#curving"
                                            }
                                        ]
                                    },
                                    "text": "curving"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strict"
                                            }
                                        ]
                                    },
                                    "text": "strict"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#defexed"
                                            }
                                        ]
                                    },
                                    "text": "defexed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#zigzag"
                                            }
                                        ]
                                    },
                                    "text": "zigzag"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#arcuate"
                                            }
                                        ]
                                    },
                                    "text": "arcuate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sinuous"
                                            }
                                        ]
                                    },
                                    "text": "sinuous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#excurved"
                                            }
                                        ]
                                    },
                                    "text": "excurved"
                                }
                            ],
                            "text": "course"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#direction"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#back"
                                            }
                                        ]
                                    },
                                    "text": "back"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#horizontally"
                                            }
                                        ]
                                    },
                                    "text": "horizontally"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#front"
                                            }
                                        ]
                                    },
                                    "text": "front"
                                }
                            ],
                            "text": "direction"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#architecture"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#serrulate-winged"
                                            }
                                        ]
                                    },
                                    "text": "serrulate-winged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glumelike"
                                            }
                                        ]
                                    },
                                    "text": "glumelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#interrupted"
                                            }
                                        ]
                                    },
                                    "text": "interrupted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nipplelike"
                                            }
                                        ]
                                    },
                                    "text": "nipplelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unwinged"
                                            }
                                        ]
                                    },
                                    "text": "unwinged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#marginal"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white-margined"
                                                    }
                                                ]
                                            },
                                            "text": "white-margined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-margined"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-margined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smooth-margined"
                                                    }
                                                ]
                                            },
                                            "text": "smooth-margined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ciliate-margined"
                                                    }
                                                ]
                                            },
                                            "text": "ciliate-margined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#serrulate-margined"
                                                    }
                                                ]
                                            },
                                            "text": "serrulate-margined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thin-margined"
                                                    }
                                                ]
                                            },
                                            "text": "thin-margined"
                                        }
                                    ],
                                    "text": "marginal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#furrowed"
                                            }
                                        ]
                                    },
                                    "text": "furrowed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gynecadrous"
                                            }
                                        ]
                                    },
                                    "text": "gynecadrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compound"
                                            }
                                        ]
                                    },
                                    "text": "compound"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unbranched"
                                            }
                                        ]
                                    },
                                    "text": "unbranched"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pedunculate",
                                                "exact synonyms": "peduncled"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-pedunculate",
                                                        "exact synonyms": "long-peduncled"
                                                    }
                                                ]
                                            },
                                            "text": "long-pedunculate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-pedunculate"
                                                    }
                                                ]
                                            },
                                            "text": "short-pedunculate"
                                        }
                                    ],
                                    "text": "pedunculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glumaceous"
                                            }
                                        ]
                                    },
                                    "text": "glumaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stoloniferous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-stoloniferous"
                                                    }
                                                ]
                                            },
                                            "text": "long-stoloniferous"
                                        }
                                    ],
                                    "text": "stoloniferous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wiry"
                                            }
                                        ]
                                    },
                                    "text": "wiry"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#splitting"
                                            }
                                        ]
                                    },
                                    "text": "splitting"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thin-winged"
                                            }
                                        ]
                                    },
                                    "text": "thin-winged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pistillate"
                                            }
                                        ]
                                    },
                                    "text": "pistillate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#septate"
                                            }
                                        ]
                                    },
                                    "text": "septate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cuplike"
                                            }
                                        ]
                                    },
                                    "text": "cuplike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bristlelike"
                                            }
                                        ]
                                    },
                                    "text": "bristlelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaflike"
                                            }
                                        ]
                                    },
                                    "text": "leaflike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#compact"
                                            }
                                        ]
                                    },
                                    "text": "compact"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#undivided"
                                            }
                                        ]
                                    },
                                    "text": "undivided"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bidendate"
                                            }
                                        ]
                                    },
                                    "text": "bidendate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#plicate"
                                            }
                                        ]
                                    },
                                    "text": "plicate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flowered"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#many-flowered"
                                                    }
                                                ]
                                            },
                                            "text": "many-flowered"
                                        }
                                    ],
                                    "text": "flowered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#winglike"
                                            }
                                        ]
                                    },
                                    "text": "winglike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spiked"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fewer-spiked"
                                                    }
                                                ]
                                            },
                                            "text": "fewer-spiked"
                                        }
                                    ],
                                    "text": "spiked"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wingless",
                                                "exact synonyms": "unwinged"
                                            }
                                        ]
                                    },
                                    "text": "wingless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bidentulate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#minutely%20bidentulate"
                                                    }
                                                ]
                                            },
                                            "text": "minutely bidentulate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#weakly%20bidentulate"
                                                    }
                                                ]
                                            },
                                            "text": "weakly bidentulate"
                                        }
                                    ],
                                    "text": "bidentulate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#phyllopodic"
                                            }
                                        ]
                                    },
                                    "text": "phyllopodic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#keeled"
                                            }
                                        ]
                                    },
                                    "text": "keeled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#branched",
                                                "exact synonyms": "branching"
                                            }
                                        ]
                                    },
                                    "text": "branched"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#folded"
                                            }
                                        ]
                                    },
                                    "text": "folded"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rhizomatous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stout-rhizomatous"
                                                    }
                                                ]
                                            },
                                            "text": "stout-rhizomatous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-rhizomatous"
                                                    }
                                                ]
                                            },
                                            "text": "short-rhizomatous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-rhizomatous"
                                                    }
                                                ]
                                            },
                                            "text": "long-rhizomatous"
                                        }
                                    ],
                                    "text": "rhizomatous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#epistomic"
                                            }
                                        ]
                                    },
                                    "text": "epistomic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lobed"
                                            }
                                        ]
                                    },
                                    "text": "lobed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#loose"
                                            }
                                        ]
                                    },
                                    "text": "loose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#collapsing"
                                            }
                                        ]
                                    },
                                    "text": "collapsing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hairlike"
                                            }
                                        ]
                                    },
                                    "text": "hairlike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#aphyllopodic"
                                            }
                                        ]
                                    },
                                    "text": "aphyllopodic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#veinless"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#essentially%20veinless"
                                                    }
                                                ]
                                            },
                                            "text": "essentially veinless"
                                        }
                                    ],
                                    "text": "veinless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#solid"
                                            }
                                        ]
                                    },
                                    "text": "solid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#congested"
                                            }
                                        ]
                                    },
                                    "text": "congested"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bractlike"
                                            }
                                        ]
                                    },
                                    "text": "bractlike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#veined"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green-veined"
                                                    }
                                                ]
                                            },
                                            "text": "green-veined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#several-veined"
                                                    }
                                                ]
                                            },
                                            "text": "several-veined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#few-veined"
                                                    }
                                                ]
                                            },
                                            "text": "few-veined"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reticulate"
                                                    }
                                                ]
                                            },
                                            "text": "reticulate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#many-veined"
                                                    }
                                                ]
                                            },
                                            "text": "many-veined"
                                        }
                                    ],
                                    "text": "veined"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spathelike"
                                            }
                                        ]
                                    },
                                    "text": "spathelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coiled"
                                            }
                                        ]
                                    },
                                    "text": "coiled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awned",
                                                "exact synonyms": "with awn"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smooth-awned"
                                                    }
                                                ]
                                            },
                                            "text": "smooth-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-awned"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acuminate-awned"
                                                    }
                                                ]
                                            },
                                            "text": "acuminate-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-awned"
                                                    }
                                                ]
                                            },
                                            "text": "long-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rough-awned"
                                                    }
                                                ]
                                            },
                                            "text": "rough-awned"
                                        }
                                    ],
                                    "text": "awned"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheathed",
                                                "exact synonyms": "ensheathed"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#aphyllous-sheathed"
                                                    }
                                                ]
                                            },
                                            "text": "aphyllous-sheathed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-sheathed",
                                                        "exact synonyms": "short-sheathing"
                                                    }
                                                ]
                                            },
                                            "text": "short-sheathed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-sheathed",
                                                        "exact synonyms": "long-sheathing"
                                                    }
                                                ]
                                            },
                                            "text": "long-sheathed"
                                        }
                                    ],
                                    "text": "sheathed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#contorted"
                                            }
                                        ]
                                    },
                                    "text": "contorted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#twisted"
                                            }
                                        ]
                                    },
                                    "text": "twisted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#decompound"
                                            }
                                        ]
                                    },
                                    "text": "decompound"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#staminate"
                                            }
                                        ]
                                    },
                                    "text": "staminate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lax"
                                            }
                                        ]
                                    },
                                    "text": "lax"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#faint",
                                                "exact synonyms": "faintly"
                                            }
                                        ]
                                    },
                                    "text": "faint"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#costate"
                                            }
                                        ]
                                    },
                                    "text": "costate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spicate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#multispicate"
                                                    }
                                                ]
                                            },
                                            "text": "multispicate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unispicate"
                                                    }
                                                ]
                                            },
                                            "text": "unispicate"
                                        }
                                    ],
                                    "text": "spicate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#condensed"
                                            }
                                        ]
                                    },
                                    "text": "condensed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#simple"
                                            }
                                        ]
                                    },
                                    "text": "simple"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#threadlike"
                                            }
                                        ]
                                    },
                                    "text": "threadlike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prophyllate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cladoprophyllate"
                                                    }
                                                ]
                                            },
                                            "text": "cladoprophyllate"
                                        }
                                    ],
                                    "text": "prophyllate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enveloped"
                                            }
                                        ]
                                    },
                                    "text": "enveloped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cleft"
                                            }
                                        ]
                                    },
                                    "text": "cleft"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#amphistomic"
                                            }
                                        ]
                                    },
                                    "text": "amphistomic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#branching"
                                            }
                                        ]
                                    },
                                    "text": "branching"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pithlike"
                                            }
                                        ]
                                    },
                                    "text": "pithlike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clasping"
                                            }
                                        ]
                                    },
                                    "text": "clasping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#empty"
                                            }
                                        ]
                                    },
                                    "text": "empty"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stipitate"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-stipitate"
                                                    }
                                                ]
                                            },
                                            "text": "short-stipitate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#estipitate"
                                                    }
                                                ]
                                            },
                                            "text": "estipitate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#substipitate"
                                                    }
                                                ]
                                            },
                                            "text": "substipitate"
                                        }
                                    ],
                                    "text": "stipitate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bifid"
                                            }
                                        ]
                                    },
                                    "text": "bifid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spinulose"
                                            }
                                        ]
                                    },
                                    "text": "spinulose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#foliaceous"
                                            }
                                        ]
                                    },
                                    "text": "foliaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#edged"
                                            }
                                        ]
                                    },
                                    "text": "edged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#needlelike"
                                            }
                                        ]
                                    },
                                    "text": "needlelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tipped"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black-tipped"
                                                    }
                                                ]
                                            },
                                            "text": "black-tipped"
                                        }
                                    ],
                                    "text": "tipped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scalelike"
                                            }
                                        ]
                                    },
                                    "text": "scalelike"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#headlike"
                                            }
                                        ]
                                    },
                                    "text": "headlike"
                                }
                            ],
                            "text": "architecture"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#height"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#taller"
                                            }
                                        ]
                                    },
                                    "text": "taller"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dwarf"
                                            }
                                        ]
                                    },
                                    "text": "dwarf"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#high"
                                            }
                                        ]
                                    },
                                    "text": "high"
                                }
                            ],
                            "text": "height"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#life_cycle"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flowering"
                                            }
                                        ]
                                    },
                                    "text": "flowering"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#marescent"
                                            }
                                        ]
                                    },
                                    "text": "marescent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#maturing"
                                            }
                                        ]
                                    },
                                    "text": "maturing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#withering"
                                            }
                                        ]
                                    },
                                    "text": "withering"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dead"
                                            }
                                        ]
                                    },
                                    "text": "dead"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#older"
                                            }
                                        ]
                                    },
                                    "text": "older"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#overwintered"
                                            }
                                        ]
                                    },
                                    "text": "overwintered"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#youngest"
                                            }
                                        ]
                                    },
                                    "text": "youngest"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#falling"
                                            }
                                        ]
                                    },
                                    "text": "falling"
                                }
                            ],
                            "text": "life_cycle"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#like-category"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awnlike"
                                            }
                                        ]
                                    },
                                    "text": "awnlike"
                                }
                            ],
                            "text": "like-category"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coloration",
                                        "exact synonyms": "color"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20black"
                                                    }
                                                ]
                                            },
                                            "text": "reddish black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20tinge"
                                                    }
                                                ]
                                            },
                                            "text": "reddish tinge"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish-brown",
                                                        "exact synonyms": "reddish-brown"
                                                    }
                                                ]
                                            },
                                            "text": "reddish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20dotted"
                                                    }
                                                ]
                                            },
                                            "text": "reddish dotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20purple",
                                                        "exact synonyms": "purple reddish"
                                                    }
                                                ]
                                            },
                                            "text": "reddish purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20reddish"
                                                    }
                                                ]
                                            },
                                            "text": "tinged reddish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20reddish"
                                                    }
                                                ]
                                            },
                                            "text": "dark reddish"
                                        }
                                    ],
                                    "text": "reddish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#copper",
                                                "exact synonyms": "copper brown"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#copper-brown",
                                                        "exact synonyms": "copper-brown"
                                                    }
                                                ]
                                            },
                                            "text": "copper brown"
                                        }
                                    ],
                                    "text": "copper"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20greenish"
                                                    }
                                                ]
                                            },
                                            "text": "pale greenish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20greenish",
                                                        "exact synonyms": "pale greenish"
                                                    }
                                                ]
                                            },
                                            "text": "light greenish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish%20yellow",
                                                        "exact synonyms": "yellow greenish"
                                                    }
                                                ]
                                            },
                                            "text": "greenish yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish%20brown",
                                                        "exact synonyms": "brown greenish"
                                                    }
                                                ]
                                            },
                                            "text": "greenish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20greenish",
                                                        "exact synonyms": "greenish silvery"
                                                    }
                                                ]
                                            },
                                            "text": "silvery greenish"
                                        }
                                    ],
                                    "text": "greenish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowish%20green",
                                                        "exact synonyms": "green yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "yellowish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowish-brown",
                                                        "exact synonyms": "yellowish-brown"
                                                    }
                                                ]
                                            },
                                            "text": "yellowish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "pale yellowish"
                                        }
                                    ],
                                    "text": "yellowish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20black",
                                                        "exact synonyms": "black brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "brownish tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20green",
                                                        "exact synonyms": "green brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20brownish"
                                                    }
                                                ]
                                            },
                                            "text": "tinged brownish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20yellow",
                                                        "exact synonyms": "yellow brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20brownish"
                                                    }
                                                ]
                                            },
                                            "text": "pale brownish"
                                        }
                                    ],
                                    "text": "brownish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pallid"
                                            }
                                        ]
                                    },
                                    "text": "pallid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lighter"
                                            }
                                        ]
                                    },
                                    "text": "lighter"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red",
                                                "exact synonyms": "red tinged"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20brown",
                                                        "exact synonyms": "red-brown"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#redbrown%20tinged",
                                                                "exact synonyms": "redbrown tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red brown tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged-red-brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged red brown"
                                                }
                                            ],
                                            "text": "red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pink"
                                                    }
                                                ]
                                            },
                                            "text": "pink"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light-redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "light red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "pale red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20red",
                                                        "exact synonyms": "red purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20redbrown",
                                                        "exact synonyms": "dark redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "dark red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20dotted",
                                                        "exact synonyms": "reddish dotted"
                                                    }
                                                ]
                                            },
                                            "text": "red dotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20spotted"
                                                    }
                                                ]
                                            },
                                            "text": "red spotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red-gold",
                                                        "exact synonyms": "red-gold"
                                                    }
                                                ]
                                            },
                                            "text": "red gold"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-red",
                                                        "exact synonyms": "red-purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#speckled%20red%20brown",
                                                        "exact synonyms": "speckled redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "speckled red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20red"
                                                    }
                                                ]
                                            },
                                            "text": "dotted red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20red"
                                                    }
                                                ]
                                            },
                                            "text": "dark red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#maroon"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20maroon"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dark maroon"
                                                }
                                            ],
                                            "text": "maroon"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wine-red"
                                                    }
                                                ]
                                            },
                                            "text": "wine-red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20tinged",
                                                        "exact synonyms": "tinged red"
                                                    }
                                                ]
                                            },
                                            "text": "red tinged"
                                        }
                                    ],
                                    "text": "red"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olivaceous"
                                            }
                                        ]
                                    },
                                    "text": "olivaceous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#clear",
                                                "exact synonyms": "colorless"
                                            }
                                        ]
                                    },
                                    "text": "clear"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rusty"
                                            }
                                        ]
                                    },
                                    "text": "rusty"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark-brown",
                                                        "exact synonyms": "dark-brown"
                                                    }
                                                ]
                                            },
                                            "text": "dark brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#medium%20brown"
                                                    }
                                                ]
                                            },
                                            "text": "medium brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown%20stained"
                                                    }
                                                ]
                                            },
                                            "text": "brown stained"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bronze"
                                                    }
                                                ]
                                            },
                                            "text": "bronze"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orangebrown",
                                                        "exact synonyms": "brown orange"
                                                    }
                                                ]
                                            },
                                            "text": "orange brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowish-brown",
                                                        "exact synonyms": "yellowish-brown"
                                                    }
                                                ]
                                            },
                                            "text": "yellowish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "pale red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#copper-brown",
                                                        "exact synonyms": "copper-brown"
                                                    }
                                                ]
                                            },
                                            "text": "copper brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous-brown",
                                                        "exact synonyms": "brown ferruginous"
                                                    }
                                                ]
                                            },
                                            "text": "ferruginous-brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20redbrown",
                                                        "exact synonyms": "dark redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "dark red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light-brown",
                                                        "exact synonyms": "light brown"
                                                    }
                                                ]
                                            },
                                            "text": "light brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20brown"
                                                    }
                                                ]
                                            },
                                            "text": "silvery brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20brown",
                                                        "exact synonyms": "brown hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#speckled%20red%20brown",
                                                        "exact synonyms": "speckled redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "speckled red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20brown"
                                                    }
                                                ]
                                            },
                                            "text": "tinged brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tan"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tan-hyaline",
                                                                "exact synonyms": "tan-hyaline"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tan hyaline"
                                                }
                                            ],
                                            "text": "tan"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-brown",
                                                        "exact synonyms": "gray-brown"
                                                    }
                                                ]
                                            },
                                            "text": "gray brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20brown",
                                                        "exact synonyms": "brown purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown-green",
                                                        "exact synonyms": "brown-green"
                                                    }
                                                ]
                                            },
                                            "text": "brown green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold-brown",
                                                        "exact synonyms": "brown gold"
                                                    }
                                                ]
                                            },
                                            "text": "gold brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20brown",
                                                        "exact synonyms": "red-brown"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#redbrown%20tinged",
                                                                "exact synonyms": "redbrown tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red brown tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged-red-brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged red brown"
                                                }
                                            ],
                                            "text": "red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light-redbrown",
                                                        "exact synonyms": "light-redbrown"
                                                    }
                                                ]
                                            },
                                            "text": "light red brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "brown tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#straw%20colored",
                                                        "exact synonyms": "stramineous"
                                                    }
                                                ]
                                            },
                                            "text": "straw colored"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#chestnut-brown",
                                                        "exact synonyms": "brown chestnut"
                                                    }
                                                ]
                                            },
                                            "text": "chestnut brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown-purple",
                                                        "exact synonyms": "brown-purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish-brown",
                                                        "exact synonyms": "reddish-brown"
                                                    }
                                                ]
                                            },
                                            "text": "reddish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black-brown",
                                                        "exact synonyms": "black-brown"
                                                    }
                                                ]
                                            },
                                            "text": "black brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grayish%20brown",
                                                        "exact synonyms": "brown grayish"
                                                    }
                                                ]
                                            },
                                            "text": "grayish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olive-brown",
                                                        "exact synonyms": "brown olive"
                                                    }
                                                ]
                                            },
                                            "text": "olive brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellow%20brown",
                                                        "exact synonyms": "yellow-brown"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20yellowbrown",
                                                                "exact synonyms": "pale yellowbrown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "pale yellow brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20yellowbrown",
                                                                "exact synonyms": "pale yellowbrown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "light yellow brown"
                                                }
                                            ],
                                            "text": "yellow brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20brown"
                                                    }
                                                ]
                                            },
                                            "text": "dotted brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish%20brown",
                                                        "exact synonyms": "brown greenish"
                                                    }
                                                ]
                                            },
                                            "text": "greenish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stained-brown"
                                                    }
                                                ]
                                            },
                                            "text": "stained brown"
                                        }
                                    ],
                                    "text": "brown"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olive",
                                                "exact synonyms": "olivaceous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olive-brown",
                                                        "exact synonyms": "brown olive"
                                                    }
                                                ]
                                            },
                                            "text": "olive brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20olive"
                                                    }
                                                ]
                                            },
                                            "text": "dark olive"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olive-green",
                                                        "exact synonyms": "olive-green"
                                                    }
                                                ]
                                            },
                                            "text": "olive green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20olive",
                                                        "exact synonyms": "light olive"
                                                    }
                                                ]
                                            },
                                            "text": "pale olive"
                                        }
                                    ],
                                    "text": "olive"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white",
                                                "exact synonyms": "whitish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white%20spotted"
                                                    }
                                                ]
                                            },
                                            "text": "white spotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green-white",
                                                        "exact synonyms": "white green"
                                                    }
                                                ]
                                            },
                                            "text": "green white"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white%20hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "white hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ivory"
                                                    }
                                                ]
                                            },
                                            "text": "ivory"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#translucent-white",
                                                        "exact synonyms": "whitish translucent"
                                                    }
                                                ]
                                            },
                                            "text": "translucent-white"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "white-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellow-white",
                                                        "exact synonyms": "yellow-white"
                                                    }
                                                ]
                                            },
                                            "text": "yellow white"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cream-colored"
                                                    }
                                                ]
                                            },
                                            "text": "cream-colored"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20white",
                                                        "exact synonyms": "silvery-white"
                                                    }
                                                ]
                                            },
                                            "text": "silvery white"
                                        }
                                    ],
                                    "text": "white"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#chestnut",
                                                "exact synonyms": "chestnut-colored"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#castaneous"
                                                    }
                                                ]
                                            },
                                            "text": "castaneous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#chestnut-brown",
                                                        "exact synonyms": "brown chestnut"
                                                    }
                                                ]
                                            },
                                            "text": "chestnut brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20chestnut"
                                                    }
                                                ]
                                            },
                                            "text": "dark chestnut"
                                        }
                                    ],
                                    "text": "chestnut"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous",
                                                "exact synonyms": "ferruginous-brown"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous-brown",
                                                        "exact synonyms": "brown ferruginous"
                                                    }
                                                ]
                                            },
                                            "text": "ferruginous-brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20ferrugineous"
                                                    }
                                                ]
                                            },
                                            "text": "pale ferrugineous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous-green",
                                                        "exact synonyms": "green ferruginous"
                                                    }
                                                ]
                                            },
                                            "text": "ferruginous green"
                                        }
                                    ],
                                    "text": "ferruginous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bluish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bluish%20green",
                                                        "exact synonyms": "green bluish"
                                                    }
                                                ]
                                            },
                                            "text": "bluish green"
                                        }
                                    ],
                                    "text": "bluish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#colored"
                                            }
                                        ]
                                    },
                                    "text": "colored"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black",
                                                "exact synonyms": "blackish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20black",
                                                        "exact synonyms": "black brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black%20purple",
                                                        "exact synonyms": "black-purple"
                                                    }
                                                ]
                                            },
                                            "text": "black purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20black"
                                                    }
                                                ]
                                            },
                                            "text": "reddish black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blue-black",
                                                        "exact synonyms": "blue-black"
                                                    }
                                                ]
                                            },
                                            "text": "blue black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black-mottled",
                                                        "exact synonyms": "mottled black"
                                                    }
                                                ]
                                            },
                                            "text": "black-mottled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black-brown",
                                                        "exact synonyms": "black-brown"
                                                    }
                                                ]
                                            },
                                            "text": "black brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20black",
                                                        "exact synonyms": "black purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish black"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-black",
                                                        "exact synonyms": "gray-black"
                                                    }
                                                ]
                                            },
                                            "text": "gray black"
                                        }
                                    ],
                                    "text": "black"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green",
                                                "exact synonyms": "greenish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green-yellow",
                                                        "exact synonyms": "yellow-green"
                                                    }
                                                ]
                                            },
                                            "text": "green yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green-white",
                                                        "exact synonyms": "white green"
                                                    }
                                                ]
                                            },
                                            "text": "green white"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown-green",
                                                        "exact synonyms": "brown-green"
                                                    }
                                                ]
                                            },
                                            "text": "brown green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowish%20green",
                                                        "exact synonyms": "green yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "yellowish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright%20green"
                                                    }
                                                ]
                                            },
                                            "text": "bright green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20green"
                                                    }
                                                ]
                                            },
                                            "text": "pale green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-green",
                                                        "exact synonyms": "gray-green"
                                                    }
                                                ]
                                            },
                                            "text": "gray green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20green",
                                                        "exact synonyms": "green whitish"
                                                    }
                                                ]
                                            },
                                            "text": "whitish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20green",
                                                        "exact synonyms": "green brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#olive-green",
                                                        "exact synonyms": "olive-green"
                                                    }
                                                ]
                                            },
                                            "text": "olive green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#glaucous-green",
                                                        "exact synonyms": "glaucous-green"
                                                    }
                                                ]
                                            },
                                            "text": "glaucous green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blue-green",
                                                        "exact synonyms": "blue-green"
                                                    }
                                                ]
                                            },
                                            "text": "blue green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20green"
                                                    }
                                                ]
                                            },
                                            "text": "dark green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous-green",
                                                        "exact synonyms": "green ferruginous"
                                                    }
                                                ]
                                            },
                                            "text": "ferruginous green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bluish%20green",
                                                        "exact synonyms": "green bluish"
                                                    }
                                                ]
                                            },
                                            "text": "bluish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20green",
                                                        "exact synonyms": "green silvery"
                                                    }
                                                ]
                                            },
                                            "text": "silvery green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#golden%20green"
                                                    }
                                                ]
                                            },
                                            "text": "gold green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grayish%20green",
                                                        "exact synonyms": "grayish green"
                                                    }
                                                ]
                                            },
                                            "text": "grayish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20green",
                                                        "exact synonyms": "pale greenish"
                                                    }
                                                ]
                                            },
                                            "text": "light green"
                                        }
                                    ],
                                    "text": "green"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blackish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-blackish",
                                                        "exact synonyms": "blackish purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple blackish"
                                        }
                                    ],
                                    "text": "blackish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#golden"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20golden"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline golden"
                                        }
                                    ],
                                    "text": "golden"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray",
                                                "exact synonyms": "grayish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-brown",
                                                        "exact synonyms": "gray-brown"
                                                    }
                                                ]
                                            },
                                            "text": "gray brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cinereous"
                                                    }
                                                ]
                                            },
                                            "text": "cinereous"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-green",
                                                        "exact synonyms": "gray-green"
                                                    }
                                                ]
                                            },
                                            "text": "gray green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery",
                                                        "exact synonyms": "silver"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "silvery brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20silvery"
                                                            }
                                                        ]
                                                    },
                                                    "text": "pale silvery"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20green",
                                                                "exact synonyms": "green silvery"
                                                            }
                                                        ]
                                                    },
                                                    "text": "silvery green"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20white",
                                                                "exact synonyms": "silvery-white"
                                                            }
                                                        ]
                                                    },
                                                    "text": "silvery white"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#silvery%20greenish",
                                                                "exact synonyms": "greenish silvery"
                                                            }
                                                        ]
                                                    },
                                                    "text": "silvery greenish"
                                                }
                                            ],
                                            "text": "silvery"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ashy"
                                                    }
                                                ]
                                            },
                                            "text": "ashy"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gray-black",
                                                        "exact synonyms": "gray-black"
                                                    }
                                                ]
                                            },
                                            "text": "gray black"
                                        }
                                    ],
                                    "text": "gray"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grayish",
                                                "exact synonyms": "grayish color"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grayish%20green",
                                                        "exact synonyms": "grayish green"
                                                    }
                                                ]
                                            },
                                            "text": "grayish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#grayish%20brown",
                                                        "exact synonyms": "brown grayish"
                                                    }
                                                ]
                                            },
                                            "text": "grayish brown"
                                        }
                                    ],
                                    "text": "grayish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coppery"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coppery%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "coppery tinged"
                                        }
                                    ],
                                    "text": "coppery"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sanguineous"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sanguineous-purple",
                                                        "exact synonyms": "purple sanguineous"
                                                    }
                                                ]
                                            },
                                            "text": "sanguineous purple"
                                        }
                                    ],
                                    "text": "sanguineous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#metallic"
                                            }
                                        ]
                                    },
                                    "text": "metallic"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellow",
                                                "exact synonyms": "yellowish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#green-yellow",
                                                        "exact synonyms": "yellow-green"
                                                    }
                                                ]
                                            },
                                            "text": "green yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale-yellow",
                                                        "exact synonyms": "pale yellowish"
                                                    }
                                                ]
                                            },
                                            "text": "pale-yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ferruginous-yellow"
                                                    }
                                                ]
                                            },
                                            "text": "ferruginous-yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greenish%20yellow",
                                                        "exact synonyms": "yellow greenish"
                                                    }
                                                ]
                                            },
                                            "text": "greenish yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellow-white",
                                                        "exact synonyms": "yellow-white"
                                                    }
                                                ]
                                            },
                                            "text": "yellow white"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellow%20brown",
                                                        "exact synonyms": "yellow-brown"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20yellowbrown",
                                                                "exact synonyms": "pale yellowbrown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "pale yellow brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light%20yellowbrown",
                                                                "exact synonyms": "pale yellowbrown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "light yellow brown"
                                                }
                                            ],
                                            "text": "yellow brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright-yellow"
                                                    }
                                                ]
                                            },
                                            "text": "bright-yellow"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20yellow",
                                                        "exact synonyms": "yellow brownish"
                                                    }
                                                ]
                                            },
                                            "text": "brownish yellow"
                                        }
                                    ],
                                    "text": "yellow"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold",
                                                "exact synonyms": "gold tinged"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20gold"
                                                    }
                                                ]
                                            },
                                            "text": "pale gold"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold-brown",
                                                        "exact synonyms": "brown gold"
                                                    }
                                                ]
                                            },
                                            "text": "gold brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "gold tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red-gold",
                                                        "exact synonyms": "red-gold"
                                                    }
                                                ]
                                            },
                                            "text": "red gold"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20gold",
                                                        "exact synonyms": "hyaline golden"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline gold"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20gold"
                                                    }
                                                ]
                                            },
                                            "text": "dark gold"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#golden%20green"
                                                    }
                                                ]
                                            },
                                            "text": "gold green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20gold",
                                                        "exact synonyms": "red-gold"
                                                    }
                                                ]
                                            },
                                            "text": "reddish gold"
                                        }
                                    ],
                                    "text": "gold"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orange"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bright%20orange"
                                                    }
                                                ]
                                            },
                                            "text": "bright orange"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#orangebrown",
                                                        "exact synonyms": "brown orange"
                                                    }
                                                ]
                                            },
                                            "text": "orange brown"
                                        }
                                    ],
                                    "text": "orange"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stramineous"
                                            }
                                        ]
                                    },
                                    "text": "stramineous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20brown",
                                                        "exact synonyms": "brown purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20dotted"
                                                    }
                                                ]
                                            },
                                            "text": "purplish dotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20purplish"
                                                    }
                                                ]
                                            },
                                            "text": "dark purplish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20tinged"
                                                    }
                                                ]
                                            },
                                            "text": "purplish tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20red",
                                                        "exact synonyms": "red purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20purplish"
                                                    }
                                                ]
                                            },
                                            "text": "dotted purplish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20black",
                                                        "exact synonyms": "black purplish"
                                                    }
                                                ]
                                            },
                                            "text": "purplish black"
                                        }
                                    ],
                                    "text": "purplish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light",
                                                "exact synonyms": "small"
                                            }
                                        ]
                                    },
                                    "text": "light"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blue"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blue-green",
                                                        "exact synonyms": "blue-green"
                                                    }
                                                ]
                                            },
                                            "text": "blue green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blue-black",
                                                        "exact synonyms": "blue-black"
                                                    }
                                                ]
                                            },
                                            "text": "blue black"
                                        }
                                    ],
                                    "text": "blue"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tawny"
                                            }
                                        ]
                                    },
                                    "text": "tawny"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish-hyaline"
                                                    }
                                                ]
                                            },
                                            "text": "whitish-hyaline"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20iridescent"
                                                    }
                                                ]
                                            },
                                            "text": "whitish iridescent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pale%20whitish"
                                                    }
                                                ]
                                            },
                                            "text": "pale whitish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20green",
                                                        "exact synonyms": "green whitish"
                                                    }
                                                ]
                                            },
                                            "text": "whitish green"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20translucent"
                                                    }
                                                ]
                                            },
                                            "text": "whitish translucent"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20mottled"
                                                    }
                                                ]
                                            },
                                            "text": "whitish mottled"
                                        }
                                    ],
                                    "text": "whitish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cinnamon-colored",
                                                "exact synonyms": "cinnamon-colored"
                                            }
                                        ]
                                    },
                                    "text": "cinnamon"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20purple"
                                                    }
                                                ]
                                            },
                                            "text": "dotted purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20purple"
                                                    }
                                                ]
                                            },
                                            "text": "dark purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black%20purple",
                                                        "exact synonyms": "black-purple"
                                                    }
                                                ]
                                            },
                                            "text": "black purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-blackish",
                                                        "exact synonyms": "blackish purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple blackish"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-red",
                                                        "exact synonyms": "red-purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple red"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sanguineous-purple",
                                                        "exact synonyms": "purple sanguineous"
                                                    }
                                                ]
                                            },
                                            "text": "sanguineous purple"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown-purple",
                                                        "exact synonyms": "brown-purple"
                                                    }
                                                ]
                                            },
                                            "text": "purple brown"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-tinged",
                                                        "exact synonyms": "purple-tinged"
                                                    }
                                                ]
                                            },
                                            "text": "purple tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple%20blotches"
                                                    }
                                                ]
                                            },
                                            "text": "purple blotches"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-banded",
                                                        "exact synonyms": "purple-banded"
                                                    }
                                                ]
                                            },
                                            "text": "purple banded"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20purple",
                                                        "exact synonyms": "purple reddish"
                                                    }
                                                ]
                                            },
                                            "text": "reddish purple"
                                        }
                                    ],
                                    "text": "purple"
                                }
                            ],
                            "text": "coloration"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#variability"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#uniform"
                                            }
                                        ]
                                    },
                                    "text": "uniform"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#differentiated"
                                            }
                                        ]
                                    },
                                    "text": "differentiated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#equal",
                                                "exact synonyms": "equaling"
                                            }
                                        ]
                                    },
                                    "text": "equal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#equaling"
                                            }
                                        ]
                                    },
                                    "text": "equaling"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#less%20than"
                                            }
                                        ]
                                    },
                                    "text": "less than"
                                }
                            ],
                            "text": "variability"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#size"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#large",
                                                "exact synonyms": "large-sized"
                                            }
                                        ]
                                    },
                                    "text": "large"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smaller"
                                            }
                                        ]
                                    },
                                    "text": "smaller"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#contracted"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rounded-contracted"
                                                    }
                                                ]
                                            },
                                            "text": "rounded-contracted"
                                        }
                                    ],
                                    "text": "contracted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tallest"
                                            }
                                        ]
                                    },
                                    "text": "tallest"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#plump"
                                            }
                                        ]
                                    },
                                    "text": "plump"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subequal"
                                            }
                                        ]
                                    },
                                    "text": "subequal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slender"
                                            }
                                        ]
                                    },
                                    "text": "slender"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enlarged"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#swelling"
                                                    }
                                                ]
                                            },
                                            "text": "swelling"
                                        }
                                    ],
                                    "text": "enlarged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reduced"
                                            }
                                        ]
                                    },
                                    "text": "reduced"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#much%20smaller"
                                            }
                                        ]
                                    },
                                    "text": "much smaller"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#constricted"
                                            }
                                        ]
                                    },
                                    "text": "constricted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#small",
                                                "exact synonyms": "little"
                                            }
                                        ]
                                    },
                                    "text": "small"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger"
                                            }
                                        ]
                                    },
                                    "text": "larger"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obtusely"
                                            }
                                        ]
                                    },
                                    "text": "obtusely"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#medium-sized"
                                            }
                                        ]
                                    },
                                    "text": "medium-sized"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#expanded"
                                            }
                                        ]
                                    },
                                    "text": "expanded"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#large-sized"
                                            }
                                        ]
                                    },
                                    "text": "large-sized"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#greater"
                                            }
                                        ]
                                    },
                                    "text": "greater"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#swollen",
                                                "exact synonyms": "swelling"
                                            }
                                        ]
                                    },
                                    "text": "swollen"
                                }
                            ],
                            "text": "size"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#with%20without%20entity"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#beakless"
                                            }
                                        ]
                                    },
                                    "text": "beakless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#nerveless"
                                            }
                                        ]
                                    },
                                    "text": "nerveless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unwinged"
                                            }
                                        ]
                                    },
                                    "text": "unwinged"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheathless",
                                                "exact synonyms": "non-sheathed"
                                            }
                                        ]
                                    },
                                    "text": "sheathless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-sheathing"
                                            }
                                        ]
                                    },
                                    "text": "short-sheathing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#peduncled"
                                            }
                                        ]
                                    },
                                    "text": "peduncled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pedunculate",
                                                "exact synonyms": "peduncled"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-pedunculate",
                                                        "exact synonyms": "long-peduncled"
                                                    }
                                                ]
                                            },
                                            "text": "long-pedunculate"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-pedunculate"
                                                    }
                                                ]
                                            },
                                            "text": "short-pedunculate"
                                        }
                                    ],
                                    "text": "pedunculate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-awned",
                                                "exact synonyms": "shortly awned"
                                            }
                                        ]
                                    },
                                    "text": "short-awned"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-sheathing"
                                            }
                                        ]
                                    },
                                    "text": "long-sheathing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#shortly%20awned"
                                            }
                                        ]
                                    },
                                    "text": "shortly awned"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bractless"
                                            }
                                        ]
                                    },
                                    "text": "bractless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blade"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bladeless"
                                                    }
                                                ]
                                            },
                                            "text": "bladeless"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20leaf-blade"
                                                    }
                                                ]
                                            },
                                            "text": "widest leaf-blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leaf-blade",
                                                        "exact synonyms": "leaves blade"
                                                    }
                                                ]
                                            },
                                            "text": "leaf-blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#larger%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "larger blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "proximal blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20leaf-blade"
                                                    }
                                                ]
                                            },
                                            "text": "distal leaf-blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#largest%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "largest blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal%20leaf-blade"
                                                    }
                                                ]
                                            },
                                            "text": "proximal leaf-blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "cauline blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "widest blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest%20bract%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "widest bract blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#longest%20bract%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "longest bract blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bladed"
                                                    }
                                                ]
                                            },
                                            "text": "bladed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bract%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "bract blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheath%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "sheath blade"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blade%20surface"
                                                    }
                                                ]
                                            },
                                            "text": "blade surface"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal%20bract%20blade"
                                                    }
                                                ]
                                            },
                                            "text": "distal bract blade"
                                        }
                                    ],
                                    "text": "blade"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#non-sheathed"
                                            }
                                        ]
                                    },
                                    "text": "non-sheathed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wingless",
                                                "exact synonyms": "unwinged"
                                            }
                                        ]
                                    },
                                    "text": "wingless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flowerless"
                                            }
                                        ]
                                    },
                                    "text": "flowerless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#leafless"
                                            }
                                        ]
                                    },
                                    "text": "leafless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awned",
                                                "exact synonyms": "with awn"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#smooth-awned"
                                                    }
                                                ]
                                            },
                                            "text": "smooth-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#scabrous-awned"
                                                    }
                                                ]
                                            },
                                            "text": "scabrous-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#acuminate-awned"
                                                    }
                                                ]
                                            },
                                            "text": "acuminate-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-awned"
                                                    }
                                                ]
                                            },
                                            "text": "long-awned"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#rough-awned"
                                                    }
                                                ]
                                            },
                                            "text": "rough-awned"
                                        }
                                    ],
                                    "text": "awned"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#toothless"
                                            }
                                        ]
                                    },
                                    "text": "toothless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheathed",
                                                "exact synonyms": "ensheathed"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#aphyllous-sheathed"
                                                    }
                                                ]
                                            },
                                            "text": "aphyllous-sheathed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-sheathed",
                                                        "exact synonyms": "short-sheathing"
                                                    }
                                                ]
                                            },
                                            "text": "short-sheathed"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-sheathed",
                                                        "exact synonyms": "long-sheathing"
                                                    }
                                                ]
                                            },
                                            "text": "long-sheathed"
                                        }
                                    ],
                                    "text": "sheathed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awnless"
                                            }
                                        ]
                                    },
                                    "text": "awnless"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-peduncled"
                                            }
                                        ]
                                    },
                                    "text": "long-peduncled"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sheathing"
                                            }
                                        ]
                                    },
                                    "text": "sheathing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#ensheathed"
                                            }
                                        ]
                                    },
                                    "text": "ensheathed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#awnlike"
                                            }
                                        ]
                                    },
                                    "text": "awnlike"
                                }
                            ],
                            "text": "with without entity"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#weight"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#light",
                                                "exact synonyms": "small"
                                            }
                                        ]
                                    },
                                    "text": "light"
                                }
                            ],
                            "text": "weight"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fixation"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#detatched"
                                            }
                                        ]
                                    },
                                    "text": "detatched"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basifixed"
                                            }
                                        ]
                                    },
                                    "text": "basifixed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#attached"
                                            }
                                        ]
                                    },
                                    "text": "attached"
                                }
                            ],
                            "text": "fixation"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fusion"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adnate"
                                            }
                                        ]
                                    },
                                    "text": "adnate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#free"
                                            }
                                        ]
                                    },
                                    "text": "free"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#connate"
                                            }
                                        ]
                                    },
                                    "text": "connate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fused"
                                            }
                                        ]
                                    },
                                    "text": "fused"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fasciated"
                                            }
                                        ]
                                    },
                                    "text": "fasciated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distinct",
                                                "exact synonyms": "distinctly"
                                            }
                                        ]
                                    },
                                    "text": "distinct"
                                }
                            ],
                            "text": "fusion"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#color%20pattern"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#multi-colored"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#suffused"
                                                    }
                                                ]
                                            },
                                            "text": "suffused"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stained"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown%20stained"
                                                            }
                                                        ]
                                                    },
                                                    "text": "brown stained"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stained-brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "stained brown"
                                                }
                                            ],
                                            "text": "stained"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#speckled"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#speckled%20red%20brown",
                                                                "exact synonyms": "speckled redbrown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "speckled red brown"
                                                }
                                            ],
                                            "text": "speckled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mottled"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#black-mottled",
                                                                "exact synonyms": "mottled black"
                                                            }
                                                        ]
                                                    },
                                                    "text": "black-mottled"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#whitish%20mottled"
                                                            }
                                                        ]
                                                    },
                                                    "text": "whitish mottled"
                                                }
                                            ],
                                            "text": "mottled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unspeckled"
                                                    }
                                                ]
                                            },
                                            "text": "unspeckled"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#coppery%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "coppery tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#yellowbrown%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "yellowbrown tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gold%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "gold tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brown%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "brown tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dark%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dark tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#brownish%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "brownish tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged-red-brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged red brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20tinge"
                                                            }
                                                        ]
                                                    },
                                                    "text": "reddish tinge"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-tinged",
                                                                "exact synonyms": "purple-tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "purple tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "purplish tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20reddish"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged reddish"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20pale"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged pale"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#redbrown%20tinged",
                                                                "exact synonyms": "redbrown tinged"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red brown tinged"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tinged%20brownish"
                                                            }
                                                        ]
                                                    },
                                                    "text": "tinged brownish"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20tinged",
                                                                "exact synonyms": "tinged red"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red tinged"
                                                }
                                            ],
                                            "text": "tinged"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hyaline%20suffused"
                                                    }
                                                ]
                                            },
                                            "text": "hyaline suffused"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spotted"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#white%20spotted"
                                                            }
                                                        ]
                                                    },
                                                    "text": "white spotted"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20spotted"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red spotted"
                                                }
                                            ],
                                            "text": "spotted"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#blotched"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple%20blotches"
                                                            }
                                                        ]
                                                    },
                                                    "text": "purple blotches"
                                                }
                                            ],
                                            "text": "blotched"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20purple"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dotted purple"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#red%20dotted",
                                                                "exact synonyms": "reddish dotted"
                                                            }
                                                        ]
                                                    },
                                                    "text": "red dotted"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purplish%20dotted"
                                                            }
                                                        ]
                                                    },
                                                    "text": "purplish dotted"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20red"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dotted red"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20purplish"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dotted purplish"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#dotted%20brown"
                                                            }
                                                        ]
                                                    },
                                                    "text": "dotted brown"
                                                },
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reddish%20dotted"
                                                            }
                                                        ]
                                                    },
                                                    "text": "reddish dotted"
                                                }
                                            ],
                                            "text": "dotted"
                                        }
                                    ],
                                    "text": "multi-colored"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#banded"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#purple-banded",
                                                        "exact synonyms": "purple-banded"
                                                    }
                                                ]
                                            },
                                            "text": "purple banded"
                                        }
                                    ],
                                    "text": "banded"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#colorless"
                                            }
                                        ]
                                    },
                                    "text": "colorless"
                                }
                            ],
                            "text": "color pattern"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fragility"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#flexible"
                                            }
                                        ]
                                    },
                                    "text": "flexible"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#pliable"
                                            }
                                        ]
                                    },
                                    "text": "pliable"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stiff",
                                                "exact synonyms": "stiffly"
                                            }
                                        ]
                                    },
                                    "text": "stiff"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stiffly"
                                            }
                                        ]
                                    },
                                    "text": "stiffly"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#delicate"
                                            }
                                        ]
                                    },
                                    "text": "delicate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stout",
                                                "exact synonyms": "stoutish"
                                            }
                                        ]
                                    },
                                    "text": "stout"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fragile"
                                            }
                                        ]
                                    },
                                    "text": "fragile"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tough"
                                            }
                                        ]
                                    },
                                    "text": "tough"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#stoutish"
                                            }
                                        ]
                                    },
                                    "text": "stoutish"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#strong"
                                            }
                                        ]
                                    },
                                    "text": "strong"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#weak"
                                            }
                                        ]
                                    },
                                    "text": "weak"
                                }
                            ],
                            "text": "fragility"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#position"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#below"
                                            }
                                        ]
                                    },
                                    "text": "below"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#middle"
                                            }
                                        ]
                                    },
                                    "text": "middle"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#exserted"
                                            }
                                        ]
                                    },
                                    "text": "exserted"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#surrounding"
                                            }
                                        ]
                                    },
                                    "text": "surrounding"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#terminal"
                                            }
                                        ]
                                    },
                                    "text": "terminal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#top"
                                            }
                                        ]
                                    },
                                    "text": "top"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apical"
                                            }
                                        ]
                                    },
                                    "text": "apical"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#low"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lower"
                                                    }
                                                ]
                                            },
                                            "text": "lower"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lowest"
                                                    }
                                                ]
                                            },
                                            "text": "lowest"
                                        }
                                    ],
                                    "text": "low"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subbasal",
                                                "exact synonyms": "sub-basal"
                                            }
                                        ]
                                    },
                                    "text": "subbasal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basal"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#basalmost"
                                                    }
                                                ]
                                            },
                                            "text": "basalmost"
                                        }
                                    ],
                                    "text": "basal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#central"
                                            }
                                        ]
                                    },
                                    "text": "central"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reaching"
                                            }
                                        ]
                                    },
                                    "text": "reaching"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial",
                                                "exact synonyms": "adaxially"
                                            }
                                        ]
                                    },
                                    "text": "adaxial"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enveloping"
                                            }
                                        ]
                                    },
                                    "text": "enveloping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sub-basal"
                                            }
                                        ]
                                    },
                                    "text": "sub-basal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#midway",
                                                "exact synonyms": "mid"
                                            }
                                        ]
                                    },
                                    "text": "midway"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hypogynous"
                                            }
                                        ]
                                    },
                                    "text": "hypogynous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enclosed"
                                            }
                                        ]
                                    },
                                    "text": "enclosed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#overtoppped"
                                            }
                                        ]
                                    },
                                    "text": "overtoppped"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subtending"
                                            }
                                        ]
                                    },
                                    "text": "subtending"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#exceeding"
                                            }
                                        ]
                                    },
                                    "text": "exceeding"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximal"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#proximalmost"
                                                    }
                                                ]
                                            },
                                            "text": "proximalmost"
                                        }
                                    ],
                                    "text": "proximal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#enclosing"
                                            }
                                        ]
                                    },
                                    "text": "enclosing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#covering"
                                            }
                                        ]
                                    },
                                    "text": "covering"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#concealing"
                                            }
                                        ]
                                    },
                                    "text": "concealing"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subequaling"
                                            }
                                        ]
                                    },
                                    "text": "subequaling"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#lateral",
                                                "exact synonyms": "laterally"
                                            }
                                        ]
                                    },
                                    "text": "lateral"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distal"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distalmost"
                                                    }
                                                ]
                                            },
                                            "text": "distalmost"
                                        }
                                    ],
                                    "text": "distal"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mid"
                                            }
                                        ]
                                    },
                                    "text": "mid"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial",
                                                "exact synonyms": "abaxially"
                                            }
                                        ]
                                    },
                                    "text": "abaxial"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cauline"
                                            }
                                        ]
                                    },
                                    "text": "cauline"
                                }
                            ],
                            "text": "position"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#width"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wide"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#widest"
                                                    }
                                                ]
                                            },
                                            "text": "widest"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#wider"
                                                    }
                                                ]
                                            },
                                            "children": [
                                                {
                                                    "data": {
                                                        "details": [
                                                            {
                                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#often%20wider"
                                                            }
                                                        ]
                                                    },
                                                    "text": "often wider"
                                                }
                                            ],
                                            "text": "wider"
                                        }
                                    ],
                                    "text": "wide"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrow",
                                                "exact synonyms": "narrowly"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrowing"
                                                    }
                                                ]
                                            },
                                            "text": "narrowing"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#much%20narrower"
                                                    }
                                                ]
                                            },
                                            "text": "much narrower"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#narrower"
                                                    }
                                                ]
                                            },
                                            "text": "narrower"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#usually%20narrower"
                                                    }
                                                ]
                                            },
                                            "text": "usually narrower"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distinctly%20narrower"
                                                    }
                                                ]
                                            },
                                            "text": "distinctly narrower"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#slightly%20narrower"
                                                    }
                                                ]
                                            },
                                            "text": "slightly narrower"
                                        }
                                    ],
                                    "text": "narrow"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fine"
                                            }
                                        ]
                                    },
                                    "text": "fine"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broad",
                                                "exact synonyms": "broadly"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broader"
                                                    }
                                                ]
                                            },
                                            "text": "broader"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#broadened"
                                                    }
                                                ]
                                            },
                                            "text": "broadened"
                                        }
                                    ],
                                    "text": "broad"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thickened",
                                                "exact synonyms": "thickly"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#spongy-thickened"
                                                    }
                                                ]
                                            },
                                            "text": "spongy-thickened"
                                        }
                                    ],
                                    "text": "thickened"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#thin"
                                            }
                                        ]
                                    },
                                    "text": "thin"
                                }
                            ],
                            "text": "width"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prominence"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prominent",
                                                "exact synonyms": "prominently"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#more%20prominent"
                                                    }
                                                ]
                                            },
                                            "text": "more prominent"
                                        }
                                    ],
                                    "text": "prominent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#evident"
                                            }
                                        ]
                                    },
                                    "text": "evident"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#distinguishable"
                                            }
                                        ]
                                    },
                                    "text": "distinguishable"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#hidden"
                                            }
                                        ]
                                    },
                                    "text": "hidden"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#impressed"
                                            }
                                        ]
                                    },
                                    "text": "impressed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obscure"
                                            }
                                        ]
                                    },
                                    "text": "obscure"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#apparent"
                                            }
                                        ]
                                    },
                                    "text": "apparent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#elevated"
                                            }
                                        ]
                                    },
                                    "text": "elevated"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#vestigial"
                                            }
                                        ]
                                    },
                                    "text": "vestigial"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#conspicuous",
                                                "exact synonyms": "conspicuously"
                                            }
                                        ]
                                    },
                                    "text": "conspicuous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obvious"
                                            }
                                        ]
                                    },
                                    "text": "obvious"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#inconspicuous",
                                                "exact synonyms": "indistinct"
                                            }
                                        ]
                                    },
                                    "text": "inconspicuous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#obsolete"
                                            }
                                        ]
                                    },
                                    "text": "obsolete"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#exposed"
                                            }
                                        ]
                                    },
                                    "text": "exposed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#raised"
                                            }
                                        ]
                                    },
                                    "text": "raised"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#indistinct"
                                            }
                                        ]
                                    },
                                    "text": "indistinct"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#indistinguishable"
                                            }
                                        ]
                                    },
                                    "text": "indistinguishable"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#visible"
                                            }
                                        ]
                                    },
                                    "text": "visible"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#concealed",
                                                "exact synonyms": "concealing"
                                            }
                                        ]
                                    },
                                    "text": "concealed"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#defined"
                                            }
                                        ]
                                    },
                                    "text": "defined"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#protruding"
                                            }
                                        ]
                                    },
                                    "text": "protruding"
                                }
                            ],
                            "text": "prominence"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#reproduction"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#unisexual"
                                            }
                                        ]
                                    },
                                    "text": "unisexual"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#female"
                                            }
                                        ]
                                    },
                                    "text": "female"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#gynecandrous"
                                            }
                                        ]
                                    },
                                    "text": "gynecandrous"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#sterile"
                                            }
                                        ]
                                    },
                                    "text": "sterile"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#monoecious"
                                            }
                                        ]
                                    },
                                    "text": "monoecious"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#bisexual"
                                            }
                                        ]
                                    },
                                    "text": "bisexual"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#vegetative"
                                            }
                                        ]
                                    },
                                    "text": "vegetative"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#fertile"
                                            }
                                        ]
                                    },
                                    "text": "fertile"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#male"
                                            }
                                        ]
                                    },
                                    "text": "male"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#androgynous"
                                            }
                                        ]
                                    },
                                    "text": "androgynous"
                                }
                            ],
                            "text": "reproduction"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#growth_form"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#caespitose"
                                            }
                                        ]
                                    },
                                    "text": "caespitose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#procumbent"
                                            }
                                        ]
                                    },
                                    "text": "procumbent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#excurrent"
                                            }
                                        ]
                                    },
                                    "text": "excurrent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#prostrate"
                                            }
                                        ]
                                    },
                                    "text": "prostrate"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#woody"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#subwoody"
                                                    }
                                                ]
                                            },
                                            "text": "subwoody"
                                        }
                                    ],
                                    "text": "woody"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#creeping"
                                            }
                                        ]
                                    },
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#short-creeping"
                                                    }
                                                ]
                                            },
                                            "text": "short-creeping"
                                        },
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#long-creeping"
                                                    }
                                                ]
                                            },
                                            "text": "long-creeping"
                                        }
                                    ],
                                    "text": "creeping"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#colonial"
                                            }
                                        ]
                                    },
                                    "text": "colonial"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#decumbent"
                                            }
                                        ]
                                    },
                                    "text": "decumbent"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#tuft-forming"
                                            }
                                        ]
                                    },
                                    "text": "tuft-forming"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#mat-forming"
                                            }
                                        ]
                                    },
                                    "text": "mat-forming"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#cespitose",
                                                "exact synonyms": "mat-forming"
                                            }
                                        ]
                                    },
                                    "text": "cespitose"
                                },
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#herbaceous"
                                            }
                                        ]
                                    },
                                    "text": "herbaceous"
                                }
                            ],
                            "text": "growth_form"
                        },
                        {
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#age"
                                    }
                                ]
                            },
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#old"
                                            }
                                        ]
                                    },
                                    "text": "old"
                                }
                            ],
                            "text": "age"
                        }
                    ],
                    "text": "quality"
                },
                sampleEntityAllFields: {
                    "children": [
                        {
                            "children": [
                                {
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#adaxial",
                                                "shared synonyms": "I'm a shared synonym",
                                                "exact synonyms": "adaxially",
                                                "elucidation": "http://biosemantics.arizona.edu/images/adaxial.jpg"
                                            }
                                        ],
                                    },
                                    "text": "adaxial"
                                },
                                {
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial-like"
                                                    }
                                                ],
                                            },
                                            "text": "abaxial-like"
                                        }
                                    ],
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial",
                                                "exact synonyms": "abaxially"
                                            }
                                        ],
                                    },
                                    "text": "abaxial"
                                }
                            ],
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#position"
                                    }
                                ],
                            },
                            "text": "position"
                        },
                        {
                            "children": [
                                {
                                    "children": [
                                        {
                                            "data": {
                                                "details": [
                                                    {
                                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial-like"
                                                    }
                                                ],
                                            },
                                            "text": "abaxial-like"
                                        }
                                    ],
                                    "data": {
                                        "details": [
                                            {
                                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#abaxial",
                                                "exact synonyms": "abaxially"
                                            }
                                        ],
                                    },
                                    "text": "abaxial"
                                }
                            ],
                            "data": {
                                "details": [
                                    {
                                        "IRI": "http://biosemantics.arizona.edu/ontologies/carex#location"
                                    }
                                ],
                            },
                            "text": "location"
                        }
                    ],
                    "data": {
                        "details": [
                            {
                                "IRI": "http://biosemantics.arizona.edu/ontologies/carex#quality",
                                "definition": "Attributes or characteristics an object has"
                            }
                        ],
                    },
                    "text": "quality"
                },
                summary: [],
            }
        },
        mounted() {
            console.log('Component mounted.')
        },
        beforeMount() {

        },
        methods: {
            submit: function(status) {
                var app = this;
                console.log("term", app.term);
                switch (status) {
                    case 0:
                        if (app.term.definition == null) {
                            alert('Please insert the human readable text.');
                        } else {
                            app.status = 1;
                            app.summary.push(app.temp.text + " is set the human readable text as " + app.term.definition + ".");
                        }
                        break;
                    case 1:
                        if (app.term.represent == null) {
                            alert("Please select the radio button for representing.")
                        } else {
                            if (app.term.represent == 'anatomical') {
                                app.treeData.push(app.largeDataEntity);
                                app.status = 2;
                                app.summary.push(app.temp.text + " is set as an anatomical structure.");
                            } else {
                                app.treeData.push(app.largeDataQuality);
                                app.status = -1;
                                app.summary.push(app.temp.text + " is set as a character.");
                            }
                        }
                        break;
                    case 2:
                        if (app.term.synonym == null) {
                            alert("Please select 'Yes' or 'No' before clicking the Save button");
                        } else {
                            if (app.term.synonym == 'yes') {

                                app.synonyms = this.$refs.tree.find({
                                    state: { checked: true }
                                    }, true);

                                app.modalShowFlag = true;


                            } else if (app.term.synonym == 'no') {
                                app.synonyms = app.$refs.tree.find({
                                    state: {
                                        checked: true
                                    }
                                }, true);
                                app.modalShowFlag = true;
                            }
                        }
                        break;
                    case -1:
                        if (app.term.synonym == null) {
                            alert("Please select 'Yes' or 'No' before clicking the Save button");
                        } else {
                            app.synonyms = app.$refs.tree.find({
                                state: {
                                    checked: true
                                }
                            }, true);
                            app.modalShowFlag = true;
                        }
                        break;
                    case 3:
                        if (app.term.instance == null) {
                            alert("Please select radio button before clicking 'Save' button");
                        } else {
                            if (app.term.instance == 'yes') {
                                app.instances = app.$refs.tree.find({
                                    state: { checked: true }
                                }, true);

                                app.modalShowFlag = true;
                            } else if (app.term.instance == 'no') {
                                app.status = 4;
                            } else if (app.term.instance == 'no-user') {
                                app.instances = app.userInstances;

                                app.modalShowFlag = true;
                            }

                        }
                        break;
                    case 4:
                        if (app.term.hasPart == null) {
                            alert("Please select radio button before clicking 'Save' button");
                        } else {
                            if (app.term.hasPart == 'yes') {
                                app.hasParts = app.$refs.tree.find({
                                    state: { checked: true }
                                }, true);

                                app.modalShowFlag = true;
                            } else if (app.term.hasPart == 'no') {
                                app.status = 5;
                            } else if (app.term.hasPart == 'no-user') {
                                app.hasParts = app.userHasParts;
                            }
                        }
                        break;
                    default:
                        break;
                }
            },
            confirmSave: function(status) {
                var app = this;

                switch (status) {
                    case 2:
                        if (app.term.synonym == 'yes') {

                            if (app.synonyms.length == 1) {
                                var jsonRequest = {
                                    "term": app.temp.text,
                                    "classIRI": app.synonyms[0].data.details[0].IRI
                                };
                                axios.post("http://shark.sbs.arizona.edu:8080/synonym", jsonRequest)
                                    .then(function(resp) {
                                        console.log("synonym resp", resp);
//                                            app.synonyms[0].data.details[0].exact_synonym = resp.data;
                                        app.synonyms[0].data.details[0].exact_synonym = app.temp.text;
                                        app.summary.push(app.temp.text + " is added as a synonym to class " + app.synonyms[0].text);
                                        console.log("tree after updated", this.$refs.tree);
                                        app.treeData = app.$refs.tree.model;
                                        app.modalShowFlag = false;
                                        app.status = 3;
                                    })
                                    .catch(function(resp) {
                                        console.log("synonym error", resp);
                                    });
                            } else {
                                app.callLoopApi(0, 'synonym', null, null, null, null, status);

                            }


                        } else if (app.term.synonym == 'no') {
                            app.callLoopApi(0, 'class', null, null, null, null, status);

                            console.log("tree after updated", this.$refs.tree);
                            app.treeData = app.$refs.tree.model;
                            app.modalShowFlag = false;
                            app.status = 3;
                        }
                        break;
                    case -1:
                        if (app.term.synonym == 'yes') {
                            if (app.synonyms.length == 1) {
                                var jsonRequest = {
                                    "term": app.temp.text,
                                    "classIRI": app.synonyms[0].data.details[0].IRI
                                };
                                axios.post("http://shark.sbs.arizona.edu:8080/synonym", jsonRequest)
                                    .then(function(resp) {
                                        console.log("synonym resp", resp);
//                                            app.synonyms[0].data.details[0].exact_synonym = resp.data;
                                        app.summary.push(app.temp.text + " is added as a synonym to class " + app.synonyms[0].text);
                                        app.synonyms[0].data.details[0].exact_synonym = app.temp.text;
                                        console.log("tree after updated", this.$refs.tree);
                                        app.treeData = app.$refs.tree.model;
                                        app.modalShowFlag = false;
                                        app.status = 5;
                                    })
                                    .catch(function(resp) {
                                        console.log("synonym error", resp);
                                    });
                            } else {
                                app.callLoopApi(0, 'synonym', null, null, null, null, status);
                            }

                        } else if (app.term.synonym == 'no') {
                            app.callLoopApi(0, 'class', null, null, null, null, status);

//                            console.log("tree after updated", this.$refs.tree);
//                            app.treeData = app.$refs.tree.model;
//                            app.modalShowFlag = false;
//                            app.status = 5;
                        }
                        break;
                    case 3:
                        if (app.term.instance == 'yes') {
                            app.TTBA = app.$refs.tree.find(app.temp.text, true);
                            console.log("TTBA", app.TTBA);
                            if (app.TTBA == null) {
                                for (var i = 0; i < app.synonyms.length; i++) {
                                    app.tempIndex = i;
                                    if (!app.synonyms[i].data.details[0].partOf) {
                                        app.synonyms[i].data.details[0].partOf = [

                                        ];
                                    }

                                    app.callLoopApi(0, 'partOf', 'synonym', i, null, null, status);

                                }
//                                console.log("tree after updated", this.$refs.tree);
//                                app.treeData = app.$refs.tree.model;
//                                var selectedNode = this.$refs.tree.find({
//                                    state: {checked: true}
//                                }, true);
//                                if (selectedNode != null) selectedNode.uncheck();
//
//                                app.modalShowFlag = false;
//
//                                app.status = 4;
                            } else if (app.TTBA.length > 0) {
                                for (var i = 0; i < app.TTBA.length; i++) {
                                    app.tempIndex = i;

                                    if (!app.TTBA[i].data.details[0].partOf) {
                                        app.TTBA[i].data.details[0].partOf = [

                                        ];
                                    }
                                    app.callLoopApi(0, 'partOf', 'TTBA', i, null, null, status);

                                }
//                                console.log("tree after updated", this.$refs.tree);
//                                app.treeData = app.$refs.tree.model;
//                                var selectedNode = this.$refs.tree.find({
//                                    state: {checked: true}
//                                }, true);
//                                if (selectedNode != null) selectedNode.uncheck();
//
//                                app.modalShowFlag = false;
//
//                                app.status = 4;

                            }
                        } else if (app.term.instance == 'no-user') {
                            var superClass = app.$refs.tree.find({
                                text: "material anatomical entity"
                            })[0];
                            app.TTBA = app.$refs.tree.find(app.temp.text, true);
                            for (var i = 0; i < app.userInstances.length; i++) {
                                var jsonClassRequest = {
                                    "term": app.userInstances[i].term,
                                    "superclassIRI": superClass.data.details[0].IRI,
                                    "definition": app.userInstances[i].definition,
                                    "elucidation": null
                                };
                                app.tempIndex = app.userInstances[i].term;
                                axios.post("http://shark.sbs.arizona.edu:8080/class", jsonClassRequest)
                                    .then(function(resp) {
                                        console.log("class resp", resp);
                                        if (resp.data == 'UNSUCCESSFULLY' || resp.data == 'NO_OPERATION') {

                                        } else {
                                            if (app.TTBA == null) {
                                                app.callLoopApi(0, 'partOf', 'no-synonym', null, resp.data, app.tempIndex, app.status);

                                            } else if (app.TTBA.length > 0) {
                                                app.callLoopApi(0, 'partOf', 'no-TTBA', null, resp.data, app.tempIndex, app.status);
                                            }
                                        }
                                    })
                                    .catch(function(resp) {
                                        console.log("class error", resp);
                                    });
                            }
//                            console.log("tree after updated", this.$refs.tree);
//                            app.treeData = app.$refs.tree.model;
//                            var selectedNode = this.$refs.tree.find({
//                                state: {checked: true}
//                            }, true);
//                            if (selectedNode != null) selectedNode.uncheck();
//
//                            app.modalShowFlag = false;



                            app.status = 4;
                        }
//                        console.log("tree after updated", this.$refs.tree);
//                        app.treeData = app.$refs.tree.model;
//                        app.modalShowFlag = false;
//
//                        app.status = 4;
                        break;

                    case 4:
                        if (app.term.hasPart == 'yes') {
                            app.TTBA = app.$refs.tree.find(app.temp.text, true);
                            console.log("TTBA", app.TTBA);
                            if (app.TTBA == null) {
                                for (var i = 0; i < app.synonyms.length; i++) {
                                    app.tempIndex = i;
                                    if (!app.synonyms[i].data.details[0].hasPart) {
                                        app.synonyms[i].data.details[0].hasPart = [

                                        ];
                                    }

                                    app.callLoopApi(0, 'hasPart', 'synonym', i, null, null, status);
                                }
//                                console.log("tree after updated", this.$refs.tree);
//                                app.treeData = app.$refs.tree.model;
//                                app.modalShowFlag = false;
//                                app.status = 5;
                            } else if (app.TTBA.length > 0) {
                                for (var i = 0; i < app.TTBA.length; i++) {
                                    app.tempIndex = i;
                                    if (!app.TTBA[i].data.details[0].hasPart) {
                                        app.TTBA[i].data.details[0].hasPart = [

                                        ];
                                    }

                                    app.callLoopApi(0, 'hasPart', 'TTBA', i, null, null, status);


                                }
//                                console.log("tree after updated", this.$refs.tree);
//                                app.treeData = app.$refs.tree.model;
//                                app.modalShowFlag = false;
//                                app.status = 5;

                            }
                        } else if (app.term.hasPart == 'no-user') {
                            var superClass = app.$refs.tree.find({
                                text: "material anatomical entity"
                            })[0];
                            app.TTBA = app.$refs.tree.find(app.temp.text, true);
                            for (var i = 0; i < app.userHasParts.length; i++) {
                                app.tempIndex = i;
                                var jsonClassRequest = {
                                    "term": app.userHasParts[i].term,
                                    "superclassIRI": superClass.data.details[0].IRI,
                                    "definition": app.userHasParts[i].definition,
                                    "elucidation": null
                                };
                                axios.post("http://shark.sbs.arizona.edu:8080/class", jsonClassRequest)
                                    .then(function(resp) {
                                        console.log("class resp", resp);
                                        if (resp.data == 'UNSUCCESSFULLY' || resp.data == 'NO_OPERATION') {

                                        } else {
                                            if (app.TTBA == null) {
                                                app.callLoopApi(0, 'hasPart', 'no-synonym', null, resp.data, app.status);

                                            } else if (app.TTBA.length > 0) {
                                                app.callLoopApi(0, 'hasPart', 'no-TTBA', null, resp.data, app.status);

                                            }
                                        }
                                    })
                                    .catch(function(resp) {
                                        console.log("class error", resp);
                                    });
                            }
//                            console.log("tree after updated", this.$refs.tree);
//                            app.treeData = app.$refs.tree.model;
//                            app.modalShowFlag = false;
//                            app.status = 5;
                        }

                        break;
                    default:
                        break;
                }


            },
            callLoopApi: function(index, key, setting = null, optionIndex = null, optionData = null, optionData2 = null, status = null) {
                var app = this;
                var jsonRequest;


                console.log("status", status);

                if (key == 'synonym') {
                    jsonRequest = {
                        "term": app.temp.text,
                        "classIRI": app.synonyms[index].data.details[0].IRI
                    };
                } else if (key == 'class') {
                    jsonRequest = {
                        "term": app.temp.text,
                        "superclassIRI": app.synonyms[index].data.details[0].IRI,
                        "definition": app.term.definition,
                        "elucidation": null
                    };
                } else if (key == 'partOf' || key == 'hasPart') {

                    if (setting == 'synonym') {
                        if (key == 'partOf') {
                            jsonRequest = {
                                "bearerIRI": app.synonyms[optionIndex].data.details[0].IRI,
                                "partIRI": app.instances[index].data.details[0].IRI
                            };
                        } else {
                            jsonRequest = {
                                "bearerIRI": app.synonyms[optionIndex].data.details[0].IRI,
                                "partIRI": app.hasParts[index].data.details[0].IRI
                            };
                        }
                    } else if (setting == 'TTBA') {
                        if (key == 'partOf') {
                            jsonRequest = {
                                "bearerIRI": app.TTBA[optionIndex].data.details[0].IRI,
                                "partIRI": app.instances[index].data.details[0].IRI
                            };
                        } else {
                            jsonRequest = {
                                "bearerIRI": app.TTBA[optionIndex].data.details[0].IRI,
                                "partIRI": app.hasParts[index].data.details[0].IRI
                            };
                        }

                    } else if (setting == 'no-synonym') {
                        jsonRequest = {
                            "bearerIRI": app.synonyms[index].data.details[0].IRI,
                            "partIRI": optionData
                        };
                        if (key == 'partOf') {
                            if (!app.synonyms[index].data.details[0].partOf) {
                                app.synonyms[index].data.details[0].partOf = [];
                            }
                        } else {
                            if (!app.synonyms[index].data.details[0].hasPart) {
                                app.synonyms[index].data.details[0].hasPart = [];
                            }
                        }

                    } else if (setting == 'no-TTBA') {
                        jsonRequest = {
                            "bearerIRI": app.TTBA[index].data.details[0].IRI,
                            "partIRI": optionData
                        };
                        if (key == 'partOf') {
                            if (!app.TTBA[index].data.details[0].partOf) {
                                app.TTBA[index].data.details[0].partOf = [];
                            }
                        } else {
                            if (!app.TTBA[index].data.details[0].hasPart) {
                                app.TTBA[index].data.details[0].hasPart = [];
                            }
                        }

                    }

                }

                axios.post("http://shark.sbs.arizona.edu:8080/" + key, jsonRequest)
                    .then(function(resp) {
                        console.log(key + " resp", resp);
                        if (resp.data == 'NO_OPERATION' || resp.data == 'UNSUCCESSFULLY') {
                            if (status == -1) {
//                                console.log("tree after updated", app.$refs.tree);
//                                var selectedNode = app.$refs.tree.find({
//                                    state: {checked: true}
//                                }, true);
//                                if (selectedNode != null) selectedNode.uncheck();
//                                app.treeData = app.$refs.tree.model;

                                app.modalShowFlag = false;
                                app.status = 5;
                            } else {
//                                console.log("tree after updated", app.$refs.tree);
//                                var selectedNode = app.$refs.tree.find({
//                                    state: {checked: true}
//                                }, true);
//                                if (selectedNode != null) selectedNode.uncheck();
//                                app.treeData = app.$refs.tree.model;

                                app.modalShowFlag = false;
                                app.status = status + 1;
                            }
//                            if (key == 'class' || key == 'synonym') {
//                                if (index < app.synonyms.length - 1) {
//                                    app.callLoopApi(index + 1, key, setting, optionIndex);
//                                } else {
//                                }
//                            } else if (key == 'partOf' || key == 'hasPart') {
//                                if (setting == 'no-synonym') {
//                                    if (index < app.synonyms.length - 1) {
//                                        app.callLoopApi(index + 1, key, setting, optionIndex, optionData);
//                                    } else {
//                                    }
//                                } else if (setting == 'no-TTBA') {
//                                    if (index < app.TTBA.length - 1) {
//                                        app.callLoopApi(index + 1, key, setting, optionIndex, optionData);
//                                    } else {
//                                    }
//                                } else {
//                                    if (key == 'partOf') {
//                                        if (index < app.instances.length - 1) {
//                                            app.callLoopApi(index + 1, key, setting, optionIndex);
//                                        } else {
//                                        }
//                                    } else {
//                                        if (index < app.hasParts.length - 1) {
//                                            app.callLoopApi(index + 1, key, setting, optionIndex);
//                                        } else {
//                                        }
//                                    }
//                                }
//                            }

                        } else {


                            if (key == 'class') {
                                app.temp.data.details[0].IRI = resp.data;
                                app.synonyms[index].append(app.temp);
                                app.summary.push(app.temp.text + " is added as a subclass of class " + app.synonyms[index].text);

                                app.treeData = app.$refs.tree.model;
                                if (index < app.synonyms.length - 1) {
                                    app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                } else {
                                    app.modalShowFlag = false;
                                    if (status == -1) {
                                        app.status = 5;
                                    } else {
                                        app.status = status + 1;
                                    }
//                                    app.treeData = app.$refs.tree.model;
                                }
                            } else if (key == 'partOf' || key == 'hasPart') {
                                if (setting == 'synonym') {

                                    if (key == 'partOf') {
                                        app.synonyms[optionIndex].data.details[0][key].push(
                                            app.instances[index].data.details[0].IRI
                                        );
                                        app.summary.push(app.synonyms[optionIndex].text + "(synonym of " + app.temp.text + ") part_of " + app.instances[index].text + " is added.");
                                        app.treeData = app.$refs.tree.model;
                                        if (index < app.instances.length - 1) {
                                            app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                        } else {
                                            app.modalShowFlag = false;
                                            if (status == -1) {
                                                app.status = 5;
                                            } else {
                                                app.status = status + 1;
                                            }
//                                            app.treeData = app.$refs.tree.model;
                                        }
                                    } else {
                                        app.synonyms[optionIndex].data.details[0][key].push(
                                            app.hasParts[index].data.details[0].IRI
                                        );
                                        app.summary.push(app.synonyms[optionIndex].text + "(synonym of " + app.temp.text + ") has_part " + app.hasParts[index].text + " is added.");
                                        app.treeData = app.$refs.tree.model;
                                        if (index < app.hasParts.length - 1) {
                                            app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                        } else {
                                            app.modalShowFlag = false;
                                            if (status == -1) {
                                                app.status = 5;
                                            } else {
                                                app.status = status + 1;
                                            }
//                                            app.treeData = app.$refs.tree.model;
                                        }
                                    }

                                } else if (setting == 'TTBA') {
                                    if (key == 'partOf') {
                                        app.TTBA[optionIndex].data.details[0][key].push(
                                            app.instances[index].data.details[0].IRI
                                        );
                                        app.summary.push(TTBA[optionIndex].text + " part_of " + app.instances[index].text + " is added.");
                                        app.treeData = app.$refs.tree.model;
                                        if (index < app.instances.length - 1) {
                                            app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                        } else {
                                            app.modalShowFlag = false;
                                            if (status == -1) {
                                                app.status = 5;
                                            } else {
                                                app.status = status + 1;
                                            }
//                                            app.treeData = app.$refs.tree.model;
                                        }
                                    } else {
                                        app.TTBA[optionIndex].data.details[0][key].push(
                                            app.hasParts[index].data.details[0].IRI
                                        );
                                        app.summary.push(TTBA[optionIndex].text + " has_part " + app.hasParts[index].text + " is added.");
                                        app.treeData = app.$refs.tree.model;
                                        if (index < app.hasParts.length - 1) {
                                            app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                        } else {
                                            app.modalShowFlag = false;
                                            if (status == -1) {
                                                app.status = 5;
                                            } else {
                                                app.status = status + 1;
                                            }
//                                            app.treeData = app.$refs.tree.model;
                                        }
                                    }

                                } else if (setting == 'no-synonym') {
                                    if (key == "partOf") {
                                        app.summary.push(app.synonyms[index].text + "(synonym of " + app.temp.text + ") part_of " + optionData2 + " is added.");
                                    } else {
                                        app.summary.push(app.synonyms[index].text + "(synonym of " + app.temp.text + ") has_part " + optionData2 + " is added.");
                                    }
                                    app.synonyms[index].data.details[0][key].push(
                                        resp.data
                                    );
                                    app.treeData = app.$refs.tree.model;
                                    if (index < app.synonyms.length - 1) {
                                        app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                    } else {
                                        app.modalShowFlag = false;
                                        if (status == -1) {
                                            app.status = 5;
                                        } else {
                                            app.status = status + 1;
                                        }
                                    }
                                } else if (setting == 'no-TTBA') {
                                    if (key == "partOf") {
                                        app.summary.push(app.TTBA[index].text + " part_of " + optionData2 + " is added.");
                                    } else {
                                        app.summary.push(app.TTBA[index].text + " has_part " + optionData2 + " is added.");
                                    }
                                    app.TTBA[index].data.details[0][key].push(
                                        resp.data
                                    );
                                    app.treeData = app.$refs.tree.model;
                                    if (index < app.TTBA.length - 1) {
                                        app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                    } else {
                                        app.modalShowFlag = false;
                                        if (status == -1) {
                                            app.status = 5;
                                        } else {
                                            app.status = status + 1;
                                        }
//                                        app.treeData = app.$refs.tree.model;
                                    }
                                }

                            } else if (key == 'synonym') {
                                app.synonyms[index].data.details[0].broad_synonym = app.temp.text;
                                app.summary.push(app.temp.text + " is added as a synonym to class" + app.synonyms[index].text);
                                app.treeData = app.$refs.tree.model;
                                if (index < app.synonyms.length - 1) {
                                    app.callLoopApi(index + 1, key, setting, optionIndex, optionData, optionData2, status);
                                } else {
                                    app.modalShowFlag = false;
                                    if (status == -1) {
                                        app.status = 5;
                                    } else {
                                        app.status = status + 1;
                                    }
//                                    app.treeData = app.$refs.tree.model;
                                }
                            }

//                            console.log("tree", app.$refs.tree.model);
//                            app.treeData = app.$refs.tree.model;

                        }
                    })
                    .catch(function(resp) {
                        console.log(key + " error", resp);
                    });
            },
            cancelSave: function() {
                var app = this;
                app.modalShowFlag = false;
            },
            onSynonymSelected: function(node) {
                var app = this;
                console.log("selected node", node);
                app.treeDetails.q2 = node.data.details;
//                app.arraySynonyms.push(node);
            },
            onCharacterSynonymSelected: function(node) {
                var app = this;
                app.treeDetails.qx = node.data.details;
            },
            onPartOfSelected: function(node) {
                var app = this;
                app.treeDetails.q3 = node.data.details;
            },
            onHasPartSelected: function(node) {
                var app = this;
                app.treeDetails.q4 = node.data.details;
            },
            onSynonymUnselected: function(node) {
                var app = this;

            },
            addUserDefinition: function() {
                var app = this;
                app.userInstances.push({
                    "term": null,
                    "definition": null
                });
            },
            addUserHasPart: function() {
                var app = this;
                app.userHasParts.push({
                    "term": null,
                    "definition": null
                });
            },
        }
    }
</script>
